...
Motivation: 结直肠癌是全球主要癌症死因，胃肠道息肉是关键前兆。现有深度学习分割方法缺乏可解释性，阻碍临床应用。
...
Conclusion: PolypSeg-GradCAM将高分割精度与可解释性结合，为可靠AI辅助结肠镜和早期结直肠癌预防迈出重要一步。
...
Method: 使用多种卷积神经网络（ResNet-18、EfficientNet-B0、SqueezeNet）开发和评估系统，平衡准确性和计算效率，并集成云基础架构和多用户框架。

Result: 最终模型分类疾病严重程度的准确率为85.4%，支持实时筛查，并具备云可扩展性和安全数据管理功能。

Conclusion: PerceptronCARE展示了AI驱动的远程医疗解决方案在扩大糖尿病视网膜病变筛查可及性方面的潜力，特别是在偏远和资源受限环境中。
...
<details>
  <summary>Details</summary>
Motivation: 传统正则化技术通常依赖启发式方法，在不同设置下效果不稳定。需要一种更可靠、有效的正则化方法来解决过拟合问题。

Method: 提出SIM框架，通过从变换后的输出重构输入来增强表示学习。为了降低计算复杂度，开发了ρSIM版本，采用补丁级特征采样和基于投影的方法重构潜在特征。

Result: 在图像分类、少样本提示学习和领域泛化等任务上，ρSIM相比基线方法取得了持续改进。实验证明ρSIM与现有正则化方法正交，能提升其效果，并在密集到密集任务和非视觉领域都有效。

Conclusion: SIM作为一种模型无关、任务无关的正则化器，可以作为即插即用模块集成到不同网络架构和任务中，有效增强表示学习能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究在具有挑战性的单轮平均梯度SAG机制下的梯度反演问题，其中每个样本的线索在单个批次平均梯度中纠缠在一起。

Method: 引入MAGIA框架，包含两个核心创新：1）闭式组合重缩放创建更紧的优化边界；2）基于动量的整批次和子集损失混合确保重建鲁棒性。

Result: 大量实验表明MAGIA显著优于先进方法，在大批次场景下实现高保真多图像重建，而先前方法在此场景下失败。

Conclusion: MAGIA在计算足迹与标准求解器相当且无需任何辅助信息的情况下，成功解决了单轮平均梯度机制下的梯度反演挑战。
...
Method: 采用解码器专用微调策略，在结合合成和真实文档的大规模数据集上微调预训练的MLLM，同时保留通用视觉特征。还开发了Misraj-DocOCR基准用于评估。

Result: Baseer显著优于现有的开源和商业解决方案，WER达到0.25，在阿拉伯文档OCR领域建立了新的最先进水平。

Conclusion: 研究证明了针对特定领域微调通用MLLM的好处，为阿拉伯语等高形态丰富语言的OCR建立了强基线。
...
<details>
  <summary>Details</summary>
Motivation: 从稀疏的InSAR时间序列数据预测未来地面变形具有挑战性，现有方法难以有效处理这种稀疏数据。

Method: 设计并实现了混合CNN-LSTM模型，专门用于同时学习生成数据张量的空间模式和时间依赖性。

Result: 与LightGBM和LASSO回归等基线模型相比，所提架构提供了显著更准确和空间一致的预测，建立了新的性能基准。

Conclusion: 研究证实了时空深度学习在高分辨率变形预测中的有效性和潜力，基线模型往往默认使用简单的持续性模式，突显了集成时空方法的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 需要系统性地评估AI模型对基本概念的理解能力，为更复杂的任务奠定基础。现有模型在位置信息和约束性问题的理解上存在不足，需要专门的评估框架。

Method: 开发Scrapbook框架，通过生成包含大量问题和广泛语言变体的数据集，专注于测试模型对基本概念的理解。

Result: 当代模型在物体识别和枚举方面表现熟练，但在理解位置信息和处理约束性问题时遇到困难。MobileVLM-V2模型显示出显著的答案不一致性，其他模型存在肯定答案偏见，在几何形状和位置信息问题上表现不佳。

Conclusion: Scrapbook框架为生成多样化数据集提供了有价值的工具，可用于系统评估和提升AI模型的性能，特别是在基本概念理解方面。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态AI系统在创意工作流程中的集成日益增加，理解视觉-语言-视觉管道中的信息损失对于评估系统局限性变得重要，但目前通过文本中介传递视觉内容时的退化程度仍缺乏量化研究。
...
Conclusion: 描述-生成瓶颈代表了当代多模态系统中可测量且一致的局限性，为系统改进提供了实证基础。
...
Method: 提出VLA-LPAF模块，使用单视角图像进行微调，在潜在空间中融合多视角观察，有效桥接视角不一致造成的差距

Result: 在CALVIN上平均提升约8%任务成功率，LIBERO上提升15%，定制仿真基准上提升30%，并在真实世界任务中展示视角自适应特性

Conclusion: VLA-LPAF能够有效提升VLA模型的视角适应性和泛化能力，为多视角视觉语言动作模型提供高效解决方案
...
Method: 提出URNet网络，包含局部-全局精炼模块来同时捕捉细粒度局部细节和长距离全局上下文，并引入基于KL散度的不确定性建模方法来提升预测可靠性。

Result: 在DSEC数据集上的大量实验表明，URNet在定性和定量评估中都一致优于现有最优方法。

Conclusion: URNet通过有效的局部-全局特征提取和不确定性建模，显著提升了事件相机立体深度估计的性能和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 子宫内膜异位症常导致慢性盆腔疼痛和可能的神经受累，但周围神经的成像仍然是一个挑战。需要一种自动、可重复的方法来详细分析神经，为子宫内膜异位症相关神经病变及其他神经受累疾病的非侵入性诊断铺平道路。

Method: Visionerves采用两阶段混合AI框架：(A)使用深度学习模型自动分割解剖结构，(B)通过符号空间推理进行纤维束追踪和神经识别。该方法通过模糊空间关系编码解剖知识，无需手动选择感兴趣区域。

Result: 在10名子宫内膜异位症（确诊或疑似）女性的腰骶神经丛应用中，Visionerves相比标准纤维束追踪方法有显著改进，Dice评分提高达25%，空间误差减少至小于5毫米。

Conclusion: 这种自动且可重复的方法能够进行详细的神经分析，为子宫内膜异位症相关神经病变以及其他神经受累疾病的非侵入性诊断开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有数据集主要来自发达国家，缺乏新兴经济体驾驶行为多样性代表的问题，同时保护驾驶员隐私。

Method: 使用定制Android应用程序采集高频率加速度计、陀螺仪、GPS数据与连续视频，所有数据源精确时间对齐，涵盖城市主干道、次要道路和高速公路三种道路类型。

Result: 构建了包含原始、处理和语义三层的结构化数据集，记录了正常、攻击性和危险三种目标驾驶行为。
...
Method: 采用多阶段渐进训练策略和高精度数据合成管道技术。模型系列包含3B到70B参数规模，其中8B和70B变体集成了长链思维推理能力。全部模型在百度昆仑P800芯片上训练，在5000芯片上实现了超过90%的扩展效率。

Result: 在通用基准测试中达到与领先开源模型相当的结果，在CCBench、SEEDBench IMG、ScienceQA和MMStar等基准上实现最先进性能。在OCR和文档理解领域表现突出（OCRBench 873分，DocVQA 94.75%），在数学推理（MathVista 78.6%）和逻辑推理任务上展示了卓越性能。

Conclusion: 该工作建立了一种有效的领域增强多模态模型开发方法学，适合多样化的企业部署场景。验证了大规模AI基础设施能够高效训练SOTA级多模态模型，为相关领域的技术发展提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: 解决深度学习去雾方法因缺乏配对的真实世界训练数据而导致的泛化能力不足问题，以及传统基于大气散射模型的物理方法在处理真实世界复杂雾霾模式时的局限性。

Method: 提出HazeFlow框架，将大气散射模型重新表述为ODE，借鉴Rectified Flow思想学习最优ODE轨迹；同时使用马尔可夫链布朗运动的非均匀雾霾生成方法来解决真实配对数据稀缺问题。

Result: 在多个真实世界去雾基准数据集上实现了最先进的性能表现。

Conclusion: HazeFlow通过ODE框架和创新的雾霾生成方法，有效提升了真实世界场景下的去雾性能，为解决去雾领域的域适应问题提供了新思路。
...
Conclusion: 压缩后的EcoWeedNet在保持高性能的同时显著提升了效率，证明了其在精准农业应用中的有效性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 医学诊断越来越多地使用多模态数据，但现有模型难以有效融合异构信息并保持对缺失模态的鲁棒性。现实世界中存在模态不平衡和缺失的挑战。

Method: 引入可学习的模态token来改进缺失感知的模态融合，将传统的单模态对比目标与融合的多模态表示相结合，使用增强的模态dropout和对比学习技术。

Result: 在大规模临床数据集上的实验表明，该方法实现了最先进的性能，特别是在只有单一模态可用的实际场景中表现优异，并能成功与CT基础模型集成。
...
<details>
  <summary>Details</summary>
Motivation: 肺栓塞分割在临床诊断中具有重要意义，但目前缺乏对不同分割架构在该任务上的系统性评估。本研究旨在填补这一空白，为PE分割提供性能基准和架构选择指导。

Method: 使用490个CTPA扫描构建密集标注数据集，在统一测试框架下评估9种CNN和ViT分割架构，比较预训练和随机初始化权重的影响。

Result: 最佳模型达到平均Dice分数0.7131，在60个测试扫描中检测到181个栓塞，有49个假阳性和28个假阴性。3D模型、CNN架构表现优于2D和ViT模型。

Conclusion: 3D U-Net with ResNet encoder是PE分割的最佳选择，中央和大栓塞分割效果满意，但远端栓塞仍具挑战性。分类预训练对分割任务可能产生负面影响。
...
<details>
  <summary>Details</summary>
Motivation: 手形在手语中具有基本音系作用，但计算方法很少明确建模手形，限制了识别准确性和语言分析。

Method: 结合解剖学启发的图结构和对比学习，分离时间动态和静态手形配置的图神经网络。

Result: 在签名序列中建立了首个结构化手形识别基准，在37个手形类别上达到46%准确率（基线方法为25%）。

Conclusion: 该方法有效解决了手形识别中的关键挑战，包括细微的类间差异和时间变化，显著提升了识别性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注不确定性量化方法，但忽略了分类任务本身对分布外检测的影响。本文旨在探索不同分类任务如何影响在胎儿超声图像中的可靠分布外检测性能。

Method: 在四个分类任务上测试了八种不确定性量化方法，分析任务选择对两种分布偏移类型（图像特征偏移和解剖特征偏移）检测性能的影响。

Result: 实验表明分布外检测性能随任务变化显著，最佳任务取决于具体的分布内外标准；分布外检测性能与弃权预测性能不完全一致。

Conclusion: 在医学图像分析中，需要根据具体下游应用来协调任务选择和不确定性策略，以实现最优性能。
...
<details>
  <summary>Details</summary>
Motivation: 在无互联网连接或GPS支持的受限资源场景下，现有视觉定位系统依赖大型图像数据库或重型3D模型不实用，而正射地理数据轻量且易获取，但相关研究较少。

Method: 创建包含16,425张无人机图像的大规模多模态数据集OrthoLoC，提出AdHoP优化技术可集成到任何特征匹配器中，通过解耦图像检索和特征匹配实现公平评估。

Result: AdHoP技术将匹配性能提升高达95%，平移误差降低高达63%，数据集有效解决了无人机图像与地理数据之间的域偏移问题。

Conclusion: OrthoLoC为基于正射地理数据的视觉定位提供了首个大规模基准，AdHoP技术显著提升了定位精度，推动了轻量级高精度定位系统的发展。
...
Method: 基于深度图像先验的补丁训练框架，通过掩码、补丁打乱和噪声避免恒等映射，使用感知损失捕获结构信息。

Result: 在MVTec-AD数据集上达到0.99 AUROC和0.60 AUPRC，在fabric数据集上达到0.98 AUROC和0.67 AUPRC，优于现有方法。

Conclusion: SSDnet在零样本设置下实现了有效的异常定位，无需外部数据或标签，对噪声和缺失像素具有鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 解决低资源语言（如孟加拉语）在视觉-语言模型中的挑战，包括配对数据稀缺、翻译对齐断裂和英语中心预训练忽略目标语言语义的问题

Method: 使用LaBSE验证的英-孟配对数据和11万双语提示合成图像训练管道，包含冻结的MaxViT视觉编码器、孟加拉语原生mBART-50解码器和轻量级桥接模块，核心创新是三损失目标函数

Result: 在Flickr30k-1k上达到BLEU-4 12.29、METEOR 27.98、BERTScore-F1 71.20；在MSCOCO-1k上达到BLEU-4 12.00、METEOR 28.14、BERTScore-F1 75.40，显著优于基线方法，并将真实-合成质心差距缩小41%

Conclusion: 提出的三损失协同方法有效改善了低资源语言的视觉-语言对齐，减少了虚假匹配，为低资源多模态建模提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 解决现有高效纯摄像头基线模型功能不完整的问题，将大规模多模态感知规划模型的能力移植到资源受限的实时自动驾驶系统中。
...
Conclusion: 证明了在资源受限环境下仍能保持完整的驾驶智能，弥合了大规模多模态感知规划模型与可部署实时自动驾驶系统之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 传统标注方法将球标注在模糊条纹前缘，存在不对称性且忽略了与速度相关的运动线索，限制了检测系统的性能。
...
<details>
  <summary>Details</summary>
Motivation: 在视频中逐帧运行开放词汇检测器虽然准确但计算成本高昂，需要一种高效的方法来减少检测器调用

Method: 在固定间隔的关键帧上调用OWLv2检测器，使用压缩域运动向量（3x3网格聚合）在中间帧传播检测结果，包含面积增长检查和可选单类切换

Result: 在ILSVRC2015-VID数据集上达到mAP@0.5=0.609，在宽松IoU阈值下接近逐帧检测性能，优于基于跟踪器的传播方法

Conclusion: 压缩域传播是减少检测器调用同时保持强零样本覆盖的实用方法，无需标签和微调
...
<details>
  <summary>Details</summary>
Motivation: 现有HDR照明估计方法在颜色准确性方面存在不足，而颜色是影响增强现实应用中虚拟物体渲染真实性的关键因素。大多数评估方法将颜色与其他照明属性混淆，需要专门研究颜色鲁棒性。

Method: 使用包含多样化照明颜色的新型HDR数据集，系统评估多种适应策略。重点测试通过预训练白平衡网络预处理输入图像的方法，并将其应用于三种最先进的照明估计方法。

Result: 实验结果表明，使用预训练白平衡网络预处理输入图像的方法在所有测试场景中都优于其他策略，显著提升了颜色鲁棒性。该方法具有通用性，适用于不同的照明估计模型。

Conclusion: 简单的预处理技术（如白平衡）可以有效提升现有HDR照明估计模型的颜色准确性，无需复杂的模型重新训练，为增强现实应用提供了实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 支票在金融系统中仍被广泛使用，但容易成为欺诈目标。传统支票字段检测依赖大量标注数据，而这类数据由于隐私和专有性问题难以获取。

Method: 使用视觉语言模型(VLM)和多模态大语言模型(MLLM)构建无需训练的框架，通过零样本学习自动检测支票中的签名、MICR线、金额、收款人等关键字段。

Result: 在包含110张不同格式支票的手工标注数据集上测试，模型表现出强大的性能和泛化能力。

Conclusion: 该框架不仅可以直接用于支票字段检测，还能作为高质量标注数据生成的引导机制，为开发专门化的实时目标检测模型奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有图表理解基准假设图表干净且查询基于事实，但现实图表常含扭曲并需复杂推理。本文旨在评估VLMs在退化图表下的表现，并解决其存在的幻觉和过度自信问题。

Method: 评估ChatGPT 4o、Claude Sonnet 4和Gemini 2.5 Pro在图表损坏和遮挡下的表现，引入CHART NOISe数据集，结合图表损坏、遮挡和反向不一致性测试，并提出质量过滤和遮挡检测等基线缓解策略。

Result: 模型在图表损坏或遮挡下性能急剧下降，幻觉（如数值捏造、趋势误解和实体混淆）更频繁，且在退化设置中仍过度自信，生成合理但无支持的解释。

Conclusion: 本文通过CHART NOISe数据集和基准测试，揭示了VLMs在图表理解中的系统脆弱性，为提升图表理解的鲁棒性和可靠性建立了严格测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 传统4D-MRI重建方法（如相位分箱或模板扫描）难以捕捉呼吸运动的时序变化，工作流程复杂且计算负担重，需要一种更高效、更准确的连续运动建模方法。

Method: 使用两个协同网络：空间解剖网络（SAN）编码连续3D解剖表示，时间运动网络（TMN）基于Transformer生成的呼吸信号产生时间一致的变形场。该方法无需模板和相位分箱。

Result: 在19名志愿者的自由呼吸数据集上验证，该方法能准确捕捉规则和不规则呼吸模式，保持血管和支气管连续性，处理时间从传统方法的5小时减少到15分钟训练时间，每帧3D体积推断时间小于1秒。

Conclusion: 该框架能准确重建任意呼吸状态的3D图像，性能优于传统方法，在4D放射治疗规划和实时自适应治疗中具有强大应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 快速移动小物体的精确跟踪是计算机视觉中的挑战性问题，特别是在体育机器人应用中，轻量级准确的跟踪系统可以提升机器人感知和规划能力

Method: 使用包含10,000个标注壁球帧的自定义数据集，评估OCSORT、DeepOCSORT、ByteTrack、BoTSORT和StrongSORT五种跟踪方法，分析推理速度和更新频率对跟踪精度的影响

Result: DeepOCSORT获得最低跟踪误差（平均ADE 31.15像素），ByteTrack处理速度最快（26.6ms），但所有跟踪器都表现出显著的空间误差（3-11cm，31-114像素）

Conclusion: 当前跟踪方法在处理快速移动小物体的不可预测运动模式方面存在根本性限制，需要专门的方法论来改进，误差率比标准物体跟踪基准高3-4倍
...
<details>
  <summary>Details</summary>
Motivation: 传统视频动作识别方法计算成本高，而压缩域处理可以显著降低计算开销。MoCrop旨在利用视频压缩中已有的运动信息来实现高效的动作识别。

Method: MoCrop包含三个轻量级组件：去噪与合并(DM)、蒙特卡洛采样(MCS)和自适应裁剪(AC)。通过运动密度子矩阵搜索定位运动密集区域，生成单个剪辑级裁剪应用于所有I帧。

Result: 在UCF101数据集上，MoCrop在同等FLOPs下提升ResNet-50准确率3.5%，或在减少26.5%FLOPs情况下提升2.4%准确率。在CoViAR上达到89.2%准确率，同时将计算量从11.6降至8.5 GFLOPs。

Conclusion: MoCrop具有强泛化能力，适用于多种骨干网络，无需训练且无额外参数，为压缩域实时视频动作识别提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在低比特率条件下表现不佳，因为它们保留了过多冗余细节或学习过度集中的符号分布，需要一种更有效的特征压缩方法

Method: 使用基于码本的自适应特征压缩框架（CAFC-SE），通过向量量化将连续视觉特征映射到离散索引，选择性传输到云端

Result: 大量实验证明该方法在速率和准确性方面具有优越性，特别是在低比特率条件下表现更稳健

Conclusion: CAFC-SE框架通过向量量化操作有效解决了低比特率条件下的特征压缩问题，为边缘-云系统提供了更高效的图像编码解决方案
...
<details>
  <summary>Details</summary>
Motivation: 针对资源受限环境下的实时医学诊断需求，需要开发计算效率高但性能优越的轻量级分割网络

Method: 设计了多核深度卷积块（MKDC）处理多尺度空间关系，并采用通道、空间和分组门控注意力机制来增强显著特征提取

Result: MK-UNet仅需0.316M参数和0.314G FLOPs，在六个医学图像基准测试中DICE分数优于TransUNet、UNeXt等SOTA方法，参数量减少333倍，性能提升达6.7%

Conclusion: MK-UNet在计算效率和分割精度方面实现了突破，为资源受限环境下的实时医学诊断提供了无与伦比的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 解决手术导航中手术视频与术前CT数据之间的不匹配问题，实现术中实时变形跟踪和术前数据的更新。

Method: 将每个高斯点相对于其父网格三角形进行参数化，强制高斯点与网格对齐，并通过光度监督联合优化高斯参数和网格变形，使变形能够传播回CT数据。

Result: 在猪内脏手术和人体肝脏模拟合成数据上验证了方法的有效性，能够在单目RGB数据上实现术前CT的合理变形。

Conclusion: BridgeSplat为可变形手术导航提供了一种有效的解决方案，能够将术中观察与术前规划数据有效结合，提高手术导航的准确性和实用性。
...
TL;DR: 本文提出了一种名为DGLE的新伪标签优化框架，用于解决遥感图像语义分割中的源自由域适应问题，通过扩散模型从少量高质量伪标签生成完整的高质量伪标签集。


<details>
  <summary>Details</summary>
Motivation: 现有的源自由域适应方法通常优化整个伪标签集，但由于伪标签包含大量噪声，同时优化所有标签具有挑战性，限制了自训练方法的性能。

Method: 首先基于置信度过滤和超分辨率增强获得少量高质量伪标签作为初始种子，然后利用扩散模型传播这些不完整分布的种子伪标签，生成完整且高质量的伪标签。

Result: 该方法有效避免了直接优化完整伪标签集的困难，显著提高了伪标签质量，从而提升了模型在目标域的性能。

Conclusion: DGLE框架通过扩散模型的强大去噪能力和复杂分布建模能力，成功解决了源自由域适应中伪标签优化的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 双曲空间相比欧几里得空间对分层数据具有更好的表示能力，特别是在粗到细少样本类增量学习任务中，需要更好地处理层次化类别关系。

Method: 使用庞加莱球模型将特征提取器嵌入双曲空间，引入双曲对比损失和双曲全连接层，并采用双曲空间最大熵分布进行特征增强以缓解少样本过拟合问题。

Result: 在C2FSCIL基准测试中，该方法有效提高了粗类和细类分类的准确率。

Conclusion: 双曲空间嵌入为粗到细少样本类增量学习提供了有效的解决方案，显著提升了模型在分层数据上的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图像外观的方法要么严格遵循掩码对齐训练而无法移除未明确掩码的因果效应，要么采用松散掩码对齐策略缺乏可控性且可能过度擦除其他物体。这些限制源于忽略了物体几何存在与其视觉效应之间的因果关系。
...
Conclusion: GeoRemover通过几何感知的方法有效解决了物体移除中的因果视觉伪影问题，实现了更完整和可控的物体移除效果。
...
Result: 在四个基准数据集上取得最先进结果：UvA-NEMO（88.7%，+0.8）、MMI（99.7%）、SPOS（98.5%，+0.7）和BBC（100%，+5.0），参数减少26%，训练过程简化。
...
<details>
  <summary>Details</summary>
Motivation: 从单目视频重建动态人体和静态场景在快速运动下存在困难，RGB帧容易产生运动模糊。事件相机具有微秒级时间分辨率，更适合动态人体重建。

Method: 使用统一的3D高斯集合，其中可学习语义属性区分人体和场景高斯。仅人体高斯进行形变动画，场景高斯保持静态。提出事件引导损失函数，匹配连续渲染间的亮度变化与事件流。

Result: 在ZJU-MoCap-Blur和MMHPSD-Blur数据集上实现了最先进的人体-场景重建效果，在PSNR/SSIM指标上显著优于基线方法，LPIPS指标更低，尤其对高速运动主体效果更好。

Conclusion: 该方法无需外部人体掩码，简化了高斯集合管理，有效解决了快速运动下的重建问题，证明了事件相机在动态场景重建中的优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时满足实时性能和决策可解释性的要求，传统监督学习和生成模型在这两方面存在不足。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在美学视觉理解方面存在局限，只能处理基础美学常识，无法满足需要专业摄影技术知识的真实场景需求。摄影美学与通用视觉理解存在本质差异，需要专门的方法来提升模型的美学分析能力。

Method: 1）构建大规模专业摄影评论数据集PhotoCritique；2）提出PhotoEye模型，采用语言引导的多视角视觉融合机制；3）建立专业美学评估基准PhotoBench。

Result: 在现有基准和新提出的PhotoBench上，PhotoEye模型相比现有模型展现出明显优势，证明了所提方法的有效性。
...
TL;DR: 提出基于XMem记忆增强架构的实时MRI引导放疗肿瘤分割框架，用于TrackRAD2025挑战，能够在长序列cine-MRI中实时跟踪肿瘤运动
...
Method: 利用XMem模型（记忆增强架构）构建肿瘤分割框架，通过高效集成记忆机制来在长序列cine-MRI中跟踪肿瘤运动

Result: 由于详细实验记录丢失，无法报告精确定量结果，但初步开发印象显示XMem框架表现出合理的分割性能并满足临床实时要求

Conclusion: 该工作有助于改进MRI引导放疗中的肿瘤跟踪精度，为癌症治疗提供更准确安全的解决方案
...
Conclusion: SSCM模型有效解决了多对比度MRI超分辨率中的空间语义一致性问题，在保持解剖结构对齐和连贯性方面表现出色
...
<details>
  <summary>Details</summary>
Motivation: 现有放射学报告生成方法依赖大规模数据和计算资源，需要多阶段训练和大型模型，导致高昂成本。本文旨在在预算受限条件下解决这一问题。

Method: 使用OraPO方法实现单阶段强化学习训练，通过轻量级oracle步骤将失败探索转化为偏好监督；FactS奖励基于临床事实提取和蕴含检查，提供密集可解释的句子级奖励。
...
TL;DR: AMSF是一个基于参考图像的无训练框架，能够在扩散模型中实现可控的多风格融合，解决了现有方法只能处理单一风格图像和缺乏多风格平衡机制的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于参考图像的方法存在两个主要限制：(a) 只能接受单个风格图像，无法实现混合美学和扩展到更多风格；(b) 缺乏平衡多种风格影响的原理性机制。
...
TL;DR: MLF-4DRCNet是一个用于3D物体检测的两阶段框架，通过4D雷达和相机图像的多级融合来解决现有方法忽略雷达点云稀疏性和不完整几何特性的问题。


<details>
  <summary>Details</summary>
Motivation: 4D毫米波雷达具有成本效益和鲁棒性，但其点云存在显著稀疏性和噪声，限制了在3D物体检测中的独立应用。现有雷达-相机融合方法大多采用为LiDAR-相机融合设计的BEV范式，忽略了雷达的固有缺陷。
...
Conclusion: MLF-4DRCNet通过点级、场景级和提案级的多模态信息融合，实现了全面的特征表示，有效解决了4D雷达点云稀疏性和噪声问题，在3D物体检测任务中表现出色。
...
TL;DR: 本文提出了一种无需训练的PDLS框架，通过双潜在流（结构路径和语义路径）解决扩散模型在图像反演中的语义漂移问题，使用LQR控制器动态引导生成轨迹。
...
Method: PDLS框架基于Rectified Flow模型，将反演过程分解为结构路径和语义路径，通过最优控制问题建模，使用LQR控制器实现动态轨迹引导。

Result: 在FFHQ-1K和ImageNet-1K数据集上的多种反演任务（高斯去模糊、运动去模糊、超分辨率和自由形式修复）中，PDLS相比单潜在基线方法能产生更忠实于原图且语义对齐更好的重建结果。

Conclusion: PDLS提供了一种有效的训练免费解决方案，通过双潜在流引导成功解决了图像反演中的语义漂移问题，在保持细节的同时确保语义准确性。
...
TL;DR: 开发了Prima，首个基于视觉语言模型的神经影像AI基础模型，用于临床MRI研究分析，在52种神经疾病诊断中达到92.0的平均AUC，优于现有最先进模型。
...
Method: 利用大型学术医疗系统作为数据引擎，训练基于22万多个MRI研究的视觉语言模型Prima，采用分层视觉架构提供通用且可迁移的MRI特征。

Result: 在包含3万MRI研究的1年系统测试中，Prima在主要神经疾病（肿瘤、炎症、感染、发育异常等）诊断中表现优异，提供可解释的鉴别诊断、工作列表优先级和临床转诊建议。

Conclusion: Prima展示了健康系统规模视觉语言模型的变革潜力，能够推进AI驱动的医疗保健发展，并在敏感群体间保持算法公平性。
...
TL;DR: 提出了一种名为Understanding-in-Generation（UiG）的新推理框架，通过将理解能力融入生成过程来提升统一模型的文本到图像生成性能。
...
Conclusion: UiG框架通过将理解能力融入生成过程，有效缓解了生成能力的局限性，为统一模型的文本到图像生成提供了新的推理方法。
...
TL;DR: 本文提出了一个用于内窥镜图像的深度估计基准测试和合成数据集EndoSynth，通过微调深度基础模型显著提升了在真实内窥镜图像上的性能。
...
Method: 1) 构建了全面的基准测试，评估最先进的深度估计模型在真实内窥镜图像上的表现；2) 创建并发布了包含真实深度和分割掩码的合成数据集EndoSynth；3) 使用合成数据集微调深度基础模型。

Result: 实验表明，使用EndoSynth合成数据集微调深度基础模型后，在大多数未见过的真实数据上的准确率显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有RGB-D显著目标检测方法要么受限于CNN的局部感受野，要么面临Transformer的二次复杂度问题，难以平衡性能与效率。状态空间模型(SSM)虽然具有线性复杂度，但直接应用于RGB-D SOD会导致局部语义不足和跨模态融合不充分。

Method: 提出LEAF-Mamba模型，包含两个核心组件：1)局部强调状态空间模块(LE-SSM)，用于捕获多尺度局部依赖；2)基于SSM的自适应融合模块(AFM)，实现互补的跨模态交互和可靠的跨模态集成。

Result: 在16个最先进的RGB-D SOD方法中，LEAF-Mamba在效能和效率方面均表现出色。此外，该方法在RGB-T SOD任务上也能取得优异性能，证明了强大的泛化能力。

Conclusion: LEAF-Mamba模型成功解决了RGB-D SOD中性能与效率的平衡问题，通过创新的局部强调和自适应融合机制，在多个基准测试中达到最优性能，并展现出良好的泛化能力。
...
TL;DR: 提出了一种结合窗口多头注意力机制和空间注意力机制的轻量级食品图像分类算法，在保持高精度的同时显著降低了计算复杂度
...
Method: 集成窗口多头注意力机制(WMHAM)和空间注意力机制(SAM)，WMHAM通过窗口划分降低计算成本，SAM自适应强调关键空间区域以提升特征表示
...
Method: 三阶段流程：1) 使用可提示的微调分割模型(SAM)进行精确发现和掩码提取；2) 通过两阶段微调的多模态大语言模型(MLLM)进行语义归因和上下文描述；3) 使用LLM作为评判者和人工评分进行MLLM评估

Result: OSDA结合了像素级精度和高层语义理解，解决了开放世界遥感解释中的关键挑战，支持跨多样化卫星图像的稳健评估

Conclusion: 该工作为动态土地覆盖监测提供了可扩展和可解释的解决方案，在自动化制图更新和大规模地球观测分析方面显示出强大潜力
...
TL;DR: 该论文介绍了LifeCLEF 2021植物识别挑战赛，旨在评估如何利用植物标本馆数据改进生物多样性丰富但数据稀缺地区的植物自动识别能力。


<details>
  <summary>Details</summary>
Motivation: 当前植物自动识别技术主要基于北美和西欧的野外照片数据，而生物多样性最丰富的热带地区数据稀缺。植物标本馆收藏了大量热带地区植物标本，可用于弥补这一数据缺口。

Method: 设计跨域分类任务，训练集包含数十万份植物标本馆标本和数千张野外照片，测试集仅包含野外照片。数据集涵盖约1000种圭亚那地盾地区的植物，并包含5种形态和功能性状数据。

Result: 通过评估参与研究团队的方法和系统，分析了利用标本馆数据改进热带地区植物识别的效果。

Conclusion: 植物标本馆数据可以作为补充资源，有效提升数据稀缺地区的植物自动识别性能，特别是在生物多样性热点地区。
...
TL;DR: 本文提出AGSwap方法解决多类别对象融合问题，并构建COF数据集进行基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成方法在融合不同类别对象时存在偏差、视觉混乱和语义不一致的问题，且缺乏全面的基准数据集。

Method: AGSwap方法包含两个核心组件：组级嵌入交换（通过特征操作融合不同概念的语义属性）和自适应组更新（基于平衡评估分数的动态优化机制）。

Result: 实验表明AGSwap在简单和复杂提示下均优于包括GPT-Image-1在内的现有最先进组合T2I方法。

Conclusion: AGSwap是一种简单而高效的多类别对象融合方法，同时提出的COF数据集为相关研究提供了重要基准。
...
<details>
  <summary>Details</summary>
Motivation: 当前深度学习植物识别主要关注少数物种，而全球有近36.9万种植物。该挑战赛旨在解决数据稀缺地区的植物识别问题，特别是生物多样性丰富的热带雨林区域。

Method: 基于包含1万种物种的数据集，主要聚焦圭亚那地盾和北亚马逊雨林区域。评估了参与研究团队开发的自动化识别系统，并与热带植物专家的性能进行对比。

Result: 挑战赛评估了不同系统的性能表现，并分析了主要成果。具体结果显示了自动化系统在数据稀缺地区的识别能力。

Conclusion: 该挑战赛为数据稀缺地区的植物识别提供了重要基准，推动了自动化识别技术在生物多样性保护中的应用，并展示了与专家性能的对比分析。
...
Method: 三阶段框架：1）利用视觉语言模型获取跨注意力图；2）利用扩散模型补充结构形状信息；3）通过注意力进化模块净化分割掩码
...
TL;DR: 提出属性提示组合（APC）框架，利用文本语义联合增强对象重识别（ReID）的判别性和泛化性，通过属性提示生成器和快慢训练策略解决单域和跨域ReID模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有ReID模型局限于单域或跨域场景，单域模型容易过拟合域特定特征，跨域模型可能抑制身份特定的判别性线索。需要同时提升判别性和泛化性。

Method: APC框架包含属性提示生成器（APG）和快慢训练策略（FSTS）。APG由语义属性字典（SAD）和提示组合模块（PCM）组成，生成判别性属性感知特征。FSTS通过快速更新流（FUS）和慢速更新流（SUS）平衡ReID特定判别性和预训练VLM的泛化知识。

Result: 在传统和领域泛化（DG）ReID数据集上的广泛实验表明，该框架在判别性和泛化性方面均优于最先进方法。

Conclusion: APC框架通过文本语义和视觉语言模型的结合，有效解决了ReID中的判别性与泛化性权衡问题，为实际应用提供了更实用的解决方案。
...
TL;DR: 本文提出了一种基于最优传输的框架OTCCLIP，用于重建图像-文本对，以防御针对CLIP模型的数据投毒和后门攻击。通过利用细粒度视觉和文本特征的最优传输距离来重新分配文本描述，并采用最优传输目标函数促进模态间和模态内的细粒度对齐。


<details>
  <summary>Details</summary>
Motivation: 现有的防御方法仅依赖图像和文本的全局表示来匹配新的文本描述，忽略了细粒度特征，可能导致错误的图像-文本对并损害CLIP预训练效果。

Method: 提出基于最优传输的细粒度视觉和文本特征集距离度量方法，基于最优传输距离重新分配文本描述，并采用最优传输目标函数促进模态间和模态内的细粒度对齐。

Result: 实验表明OTCCLIP能成功降低投毒攻击的成功率，相比之前的方法显著提高了CLIP在投毒数据集上的零样本和线性探测性能。

Conclusion: OTCCLIP框架通过最优传输方法有效解决了细粒度特征对齐问题，提高了CLIP模型对数据投毒攻击的鲁棒性。
...
TL;DR: 提出了LFI框架，通过建模视觉理解作为交互过程来解决视觉基础模型与视觉语言模型之间的知识转移限制


<details>
  <summary>Details</summary>
Motivation: 现有视觉基础模型主要采用结果导向范式，忽视了视觉语言模型中编码的跨模态交互过程，这种表示差异阻碍了有效的知识转移和跨视觉任务的泛化
...
Conclusion: LFI框架通过显式建模交互过程，实现了更忠实和高效的知识转移，在认知对齐方面表现优异，为视觉基础模型的发展提供了新方向
...
<details>
  <summary>Details</summary>
Motivation: 解决RGB-热成像显著目标检测中由于特征融合不足和数据稀缺导致的边界不精确和对象不完整问题

Method: 1. 动态融合网络(DFNet)生成初始显著图作为视觉提示；2. 即插即用优化网络(P2RNet)使用混合提示指导SAM优化显著图
...
<details>
  <summary>Details</summary>
Motivation: LiDAR点云在自动驾驶和机器人感知中易受噪声、遮挡和对抗性破坏的影响，传统自编码器在真实世界挑战性条件下性能下降。

Method: 提出TriFusion-AE多模态融合框架，通过交叉注意力机制对齐文本语义线索、图像几何特征和LiDAR空间结构，学习对随机噪声和对抗扰动具有鲁棒性的表示。

Result: 在nuScenes-mini数据集上评估，模型在强对抗攻击和重噪声条件下表现出显著更鲁棒的重建能力，而CNN自编码器在这些条件下崩溃。

Conclusion: 该多模态融合框架是模型无关的，可与任何基于CNN的点云自编码器无缝集成进行联合表示学习，为低数据部署场景提供有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法假设工具库固定不变，难以适应现实环境中工具数据持续演变的场景。

Method: COLT方法包含可学习的工具代码本作为工具专用记忆系统，根据用户指令与工具特征的相似度动态选择相关工具。

Result: 在视频LLM基准测试和VideoToolBench数据集上的实验表明COLT达到了最先进的性能。

Conclusion: COLT方法成功增强了开源视频LLMs的持续工具使用能力，在动态工具环境中表现优异。
...
TL;DR: FixingGS是一种无需训练的3D高斯溅射增强方法，利用扩散模型解决稀疏视角3D重建中的伪影和多视角一致性问题。


<details>
  <summary>Details</summary>
Motivation: 稀疏视角下的3D重建由于视觉信息不足会产生明显伪影，现有基于生成先验的方法难以保证多视角一致性，导致结构模糊和细节不真实。

Method: 提出蒸馏方法提供更准确且跨视角一致的扩散先验，结合自适应渐进增强方案，在约束不足区域细化重建结果。

Result: 大量实验表明FixingGS在视觉质量和重建性能上优于现有最先进方法。

Conclusion: FixingGS通过有效利用扩散模型能力，成功解决了稀疏视角3D高斯溅射重建中的伪影和多视角一致性问题。
...
TL;DR: 该论文提出Bi-VLM方法，通过基于高斯分位数的非均匀权重分离和显著性感知混合量化算法，实现视觉语言模型的超低位（≤2位）量化，显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型的计算需求和内存要求过高，限制了其在硬件受限环境中的应用，需要开发更高效的超低位量化方法。

Method: 提出Bi-VLM方法：1）基于高斯分位数非均匀分离模型权重为异常值（显著）和多个正常值（不显著）子集；2）设计显著性感知混合量化算法，根据显著性度量和压缩目标对缩放器和二进制矩阵施加不同约束。

Result: 在视觉问答任务上，Bi-VLM在语言模型部分比现有最佳方法提升3%-47%，在整个VLM上提升4%-45%。量化模型存在90%-99%的图像令牌冗余，可进一步剪枝提升效率。

Conclusion: Bi-VLM成功实现了视觉语言模型的超低位量化，显著提升了计算效率，同时保持了模型性能，为硬件受限环境下的VLM应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法依赖复杂架构、解剖学先验或精心调优的数据增强，限制了可扩展性和泛化性，且容易在解剖相似度高、病理细微的医学图像中发生捷径学习。
...
Conclusion: DiSSECT框架通过离散表示瓶颈有效解决了医学图像自监督学习中的捷径学习问题，实现了高效、可迁移的临床表示学习。
...
TL;DR: 本文提出了一种结合热成像、深度学习和车联网通信的实时检测系统，用于减少鹿车碰撞事故。


<details>
  <summary>Details</summary>
Motivation: 美国每年发生约210万起鹿车碰撞事故，造成440人死亡、5.9万人受伤和100亿美元经济损失，同时导致鹿群数量下降。

Method: 使用热成像技术和深度学习算法，在收集的12,000张热成像鹿图像数据集上进行训练和验证，并通过CV2X通信设备向周围车辆和路边单元广播预警信息。

Result: 系统表现优异，平均精度达98.84%，精确率95.44%，召回率95.96%。在恶劣天气条件下检测准确率保持在88-92%，而传统可见光相机效果低于60%。端到端延迟始终低于100毫秒。

Conclusion: 该系统通过热成像和车联网技术为减少鹿车碰撞提供了可行的技术路径，现场测试验证了其在各种天气条件下的稳健性能。
...
<details>
  <summary>Details</summary>
Motivation: 手术数据稀缺限制了深度学习系统在计算机辅助干预中的发展，现有扩散模型存在数据记忆化问题，导致样本不一致或缺乏多样性，可能损害下游性能
...
TL;DR: 提出了一种基于模型的神经网络KMDS-Net，用于动态PET图像去噪，通过利用帧间空间相关性和帧内结构一致性，在模拟和真实数据上表现出优于基线方法的去噪性能。


<details>
  <summary>Details</summary>
Motivation: 动态PET中的短时间帧由于统计量有限，图像质量较差。深度学习在医学图像去噪中显示出潜力，但需要专门针对动态PET特性的方法。

Method: 建立基于核空间的多维稀疏(KMDS)模型，利用动态PET的帧间空间相关性和帧内结构一致性。然后用神经网络替代参数估计过程，形成端到端的KMDS-Net。
...
TL;DR: 提出了一种结合光流分割标签插值和多任务学习的新框架，用于解决机器人辅助手术中时空标注不平衡问题，提高手术场景理解的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 机器人辅助手术需要精确理解视觉数据，但现有方法多为单任务方法，且存在时空标注不平衡问题（长期标注每帧都有，短期标注只在关键帧有），限制了全面理解手术场景。

Method: 使用光流估计从标注的关键帧传播标签到相邻未标注帧，通过标签插值丰富稀疏的空间监督，结合多任务学习平衡时空信息进行训练。

Result: 该框架提高了手术场景理解的准确性和效率，增强了机器人辅助手术的实用性。

Conclusion: 提出的光流标签插值与多任务学习结合的方法有效解决了手术场景理解中的时空标注不平衡问题，为机器人辅助手术提供了更好的视觉理解支持。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态上下文整合越来越多的交错多模态token，扩散去噪和自回归解码的迭代过程带来了巨大的计算开销，需要高效的加速解决方案

Method: 采用分而治之策略，使用推测解码进行下一个token预测，并通过多阶段蒸馏过程加速扩散去噪

Result: 在多模态理解任务上实现2倍以上加速；在生成任务中，无损6-NFE模型实现文本到图像生成16.67倍加速和图像编辑22倍加速；高效1-NFE模型支持近实时交互式编辑和生成

Conclusion: 通过结合先进的对抗蒸馏和人类反馈学习，该框架实现了极致的成本效益和响应性，使复杂的多模态交互变得无缝且即时
...
TL;DR: 评估多模态大语言模型在基督教圣像学单标签分类任务中的表现，发现Gemini-2.5 Pro和GPT-4o优于ResNet50基线，提示优化能提升零样本性能，支持LLMs在文化遗产领域的应用。


<details>
  <summary>Details</summary>
Motivation: 评估通用多模态大语言模型是否能够解释通常由监督分类器处理的基督教圣像学，并分析其在图像分类任务中的性能表现。

Method: 使用三个支持Iconclass的数据集（ArtDL、ICONCLASS、Wikidata），在三种条件下测试模型：仅类标签分类、带Iconclass描述的分类、五样本少样本学习。与在相同数据集上微调的ResNet50基线进行比较。

Result: Gemini-2.5 Pro和GPT-4o优于ResNet50基线。在Wikidata数据集上准确率显著下降，Siglip达到最高准确率。添加类别描述通常能改善零样本性能，少样本学习效果较差。

Conclusion: 通用多模态大语言模型能够在视觉复杂的文化遗产领域进行分类，支持将LLMs作为数字人文学科工作流中的元数据管理工具应用。
...
Conclusion: LRGC提供了一种可学习的、无需超参数的图构建方法，能够自适应地为每个节点选择最佳邻域，显著提升了视觉图神经网络的性能。
...
TL;DR: 本文提出结构化反思方法，将错误修复路径转化为明确可控的可训练动作，通过Reflect-Call-Final策略优化工具调用，显著提升多轮工具调用成功率和错误恢复能力。


<details>
  <summary>Details</summary>
Motivation: 当前工具增强的大语言模型训练方法依赖监督模仿或粗粒度强化学习，自我反思实践基于启发式提示或单向推理，在多轮交互中脆弱且容易重复犯错。

Method: 提出结构化反思方法，结合DAPO和GSPO目标与针对工具使用的奖励方案，优化Reflect-Call-Final逐步策略，并引入Tool-Reflection-Bench基准进行程序化评估。

Result: 在BFCL v3和Tool-Reflection-Bench上的实验显示，多轮工具调用成功率和错误恢复能力大幅提升，冗余调用减少。

Conclusion: 使反思明确化并直接优化可提高工具交互的可靠性，为智能体从失败中学习提供了可复现的路径。
...
TL;DR: 提出了Point Prompt Defender框架，采用攻击-防御的对抗强化学习范式，自动优化SAM模型的点提示，提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖启发式或手动设计的提示，限制了可扩展性和泛化能力，需要自动化的提示优化方法。

Method: 构建任务无关的点提示环境，将图像块表示为双空间图中的节点；使用攻击者代理学习破坏SAM性能的提示，防御者代理学习抑制这些干扰提示；基于DQN训练，奖励信号基于分割质量变化。

Result: 实验表明该方法有效提升了SAM的鲁棒性和泛化能力，建立了灵活、可解释、即插即用的提示优化框架。

Conclusion: Point Prompt Defender为基于提示的分割提供了一个有效的自动化优化解决方案，无需重新训练即可提升SAM性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决濒危物种研究、保护生态学和栖息地管理中的关键需求，为全面的环境监测提供多模态AI研究支持。

Method: 在俄亥俄州野生动物园220英亩牧场进行为期四天的同步监测，收集三种模态数据，并建立可重复的多模态野生动物监测协议。

Result: 提供了传感器模态性能的比较分析，展示了在土地利用模式、物种检测、行为分析和栖息地监测方面的互补优势。

Conclusion: 这项工作为多模态野生动物监测建立了可重复的协议，同时贡献开放数据集以推进保护计算机视觉研究，未来版本将包括GPS跟踪数据和扩展的时间覆盖范围。
...
Method: 构建了包含文本描述的遥感图像-深度图配对数据集，并基于稳定扩散模型开发了遥感深度估计模型，利用其多模态融合能力。

Result: 提出的深度估计模型在RS3DBench数据集上实现了最先进的性能。

Conclusion: RS3DBench将为遥感领域的3D视觉感知模型和地理人工智能发展做出重要贡献，数据集、模型和代码已公开。
...
TL;DR: DeblurSplat：首个无需SfM的基于事件相机的去模糊3D高斯泼溅方法，通过DUSt3R直接获取初始点云并利用事件流进行精细监督


<details>
  <summary>Details</summary>
Motivation: 解决运动模糊问题，避免传统SfM方法中相机位姿累积误差对初始点云位置的影响

Method: 1）利用DUSt3R密集立体模块从模糊图像直接获取准确初始点云；2）引入事件流进行去模糊，通过解码潜在清晰图像提供精细监督信号

Result: 在多种场景下的实验表明，DeblurSplat不仅能生成高保真新视角，而且在渲染效率上显著优于现有去模糊3D-GS方法

Conclusion: 该方法成功实现了无需SfM的高效去模糊3D场景重建，在质量和效率方面均优于现有技术
...
TL;DR: 提出MoiréNet，一种基于U-Net的卷积神经网络框架，通过协同整合频域和空域特征来有效去除图像中的莫尔条纹伪影
...
Result: 在公开和实际使用数据集上的广泛实验表明，MoiréNet实现了最先进的性能，同时具有高参数效率

Conclusion: MoiréNet仅需5.513M参数（比ESDNet-L减少48%），将卓越的恢复质量与参数效率相结合，非常适合智能手机摄影、工业成像和增强现实等资源受限应用
...
TL;DR: 本文提出了一种新颖的频率感知音频-视觉分割框架FAVS，通过频率域分解和重组来解决音频和视觉模态在频率域中的固有矛盾，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视觉分割方法忽略了音频和视觉模态在频率域中的固有矛盾——音频高频信号普遍存在干扰噪声，而视觉高频信号包含丰富的结构细节。忽略这些差异会导致次优性能。

Method: FAVS框架包含两个关键模块：频率域增强分解器模块（使用残差迭代频率分解来区分模态特定语义和结构特征）和协同跨模态一致性模块（采用专家混合架构通过动态专家路由增强语义一致性和模态特定特征保留）。

Result: 在三个基准数据集上的大量实验表明，FAVS框架实现了最先进的性能，丰富的定性可视化进一步验证了所提模块的有效性。

Conclusion: 将AVS任务重新表述为频率域分解和重组问题是一个有效的解决方案，提出的FAVS框架通过频率感知方法显著提升了音频-视觉分割的性能。
...
TL;DR: 本文综述了可解释人工智能（xAI）在视觉感知任务中的四种代表性方法，分析了它们的机制、优缺点及评估指标，为未来研究和应用提供指导。
...
Method: 调查了四种xAI方法：显著性图、概念瓶颈模型、基于原型的方法和混合方法，并分析其工作机制。

Result: 提供了对四种xAI方法的全面分析，包括各自的优势和局限性，以及适用的评估指标。

Conclusion: 该综述为xAI在视觉感知领域的研究和应用提供了有价值的指导，有助于推动可解释AI的发展。
...
<details>
  <summary>Details</summary>
Motivation: 真实LiDAR数据收集耗时且易受噪声和稀疏性影响（如恶劣天气或传感器限制），需要合成数据来提升自动驾驶感知性能。

Method: 使用增强的DDPM模型，包含新颖的噪声调度和时间步嵌入技术，改进去噪过程和时间感知能力，生成更真实的点云数据。
...
TL;DR: AGSSP是一种新颖的自监督预训练范式，通过异常先验指导表示学习，解决金属表面缺陷检测中预训练-微调范式的领域差距问题。该方法使用两阶段框架：首先通过异常图蒸馏知识预训练骨干网络，然后使用伪缺陷框预训练检测器。实验显示AGSSP相比ImageNet预训练模型在mAP指标上提升显著。


<details>
  <summary>Details</summary>
Motivation: 解决金属表面缺陷检测中预训练-微调范式的关键困境：自然图像预训练存在领域差距，而传统自监督预训练无法区分细微缺陷模式与复杂背景噪声。

Method: 提出异常引导的自监督预训练(AGSSP)，采用两阶段框架：1）通过异常图知识蒸馏预训练骨干网络捕获缺陷显著特征；2）使用异常图生成的伪缺陷框预训练检测器。开发知识增强方法生成高质量异常图，并收集12万张工业图像数据集。

Result: 广泛实验表明AGSSP在各种设置下持续提升性能，相比基于ImageNet的模型，mAP@0.5提升高达10%，mAP@0.5:0.95提升11.4%。

Conclusion: AGSSP通过异常先验指导表示学习，有效解决了工业缺陷检测中的预训练困境，显著提升了检测性能，所有代码、预训练模型和数据集均已开源。
...
Method: 使用跨身份多视角视频训练UHAP，通过中性扫描数据进行监督以捕捉身份特定细节。构建通用语音模型将原始音频输入直接映射到UHAP潜在表达空间，该空间编码几何和外观变化。采用单目编码器进行高效个性化，通过UHAP解码生成逼真头像。

Result: 该方法能够生成具有精确唇部同步和细腻表情细节的高度逼真头像，在唇同步准确性、图像质量和感知真实性等指标上优于现有几何方法。

Conclusion: 本文提出的方法是首个可泛化的音频驱动头像模型，能够进行详细的外观建模和渲染，在多个评估指标上均表现出色。
...
TL;DR: 提出了一种基于机器学习的模块化管道，用于自动化检测、跟踪和提取树突棘在3D+时间显微镜数据中的特征，解决了大规模分析树突棘结构动力学的挑战。


<details>
  <summary>Details</summary>
Motivation: 树突棘是大脑兴奋性突触的关键结构组成部分，其大小可作为突触效能的指标。然而，在3D+时间显微镜数据中对树突棘结构动力学进行大规模分析仍然具有挑战性且劳动密集。

Method: 采用模块化机器学习管道，包括基于transformer的检测模块、集成空间特征的深度跟踪组件、利用空间一致性跨时间关联3D棘的时间跟踪模块，以及量化生物学相关棘特征的特征提取单元。

Result: 在开源标记的棘数据和两个补充注释数据集上验证了方法，并发布了首个此类时间跟踪数据。发布了数据、代码和预训练权重，为可扩展的端到端树突棘动力学分析建立了基准。

Conclusion: 该工作提供了一个自动化、可扩展的解决方案，有助于促进树突棘动力学研究，并为未来研究建立了重要基准。
...
Method: 基于置信度的伪标签策略，在测试数据上直接训练轻量级分类器。VLM识别高置信度样本，预训练视觉模型增强其视觉表示，通过迭代训练捕获互补的语义和视觉线索。
...
TL;DR: 提出MirrorScene3D数据集和ReflectiveGS方法，解决镜像环境中3D重建和视图合成的挑战，通过利用镜像反射作为补充视角来提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理含镜面环境时性能下降，主要关注镜面对称映射而忽略了镜像反射携带的丰富信息，这些反射可以提供补充视角来填补缺失细节。

Method: 基于3D高斯泼溅(3DGS)提出ReflectiveGS方法，将镜像反射作为互补视角而非简单对称伪影，增强场景几何并恢复缺失细节。

Result: 在MirrorScene3D数据集上的实验显示，ReflectiveGS在SSIM、PSNR、LPIPS指标和训练速度上均优于现有方法。

Conclusion: 该方法为镜像丰富环境中的3D重建设定了新的基准，证明了利用镜像反射信息的有效性。
...
TL;DR: 本文提出了一种基于深度学习的胆道定位方法，利用Yolo检测算法和GAN生成合成训练数据，旨在提高腹腔镜胆囊切除术中胆管的可视化效果，减少胆管损伤风险。


<details>
  <summary>Details</summary>
Motivation: 腹腔镜胆囊切除术虽然恢复快、美容效果好，但存在较高的胆管损伤风险，严重影响患者生活质量和生存率。因此需要改进术中胆管可视化技术。

Method: 构建并标注图像数据库训练Yolo检测算法，采用经典数据增强技术和生成对抗网络(GAN)生成合成训练数据集。

Result: 实验结果表明该方法能够有效定位胆道结构，讨论了实验结果和伦理考量。

Conclusion: 深度学习技术可以有效提高腹腔镜手术中胆管的可视化效果，为减少胆管损伤提供技术支持。
...
TL;DR: 提出Prompt-DAS框架，用于电子显微镜图像中多器官实例的自适应分割，支持无监督、弱监督和交互式分割，通过提示机制和对比学习提升性能


<details>
  <summary>Details</summary>
Motivation: 解决大规模电子显微镜图像中多器官实例分割的标注效率问题，利用SAM的提示机制实现灵活的自适应学习

Method: 基于SAM构建可提示的多任务框架，结合辅助中心点检测任务和提示引导的对比学习，支持不同提示配置（全点、稀疏点、无点）

Result: 在多个挑战性基准测试中表现出优于现有UDA、WDA和SAM方法的性能

Conclusion: Prompt-DAS框架为电子显微镜图像分割提供了一种灵活高效的解决方案，显著提升了自适应学习能力
...
Result: 实验表明，包括专有模型在内的最先进MLLMs在VIR-Bench上表现不佳，突显了处理扩展时空尺度视频的难度。

Conclusion: VIR-Bench不仅能有效评估模型性能，还能转化为面向用户应用的具体性能提升，验证了评估协议的实际价值。
...
TL;DR: 本文提出了一种针对视觉语言模型的细粒度推理框架CoS，通过步骤级推理数据、过程奖励模型和强化学习训练，实现了对中间推理步骤的准确评估和有效优化。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言推理方法通常在粗粒度层面进行推理链构建，难以进行细粒度结构化推理，且难以评估中间推理步骤的质量和奖励。

Method: 提出了一个简单有效的透明框架，包括步骤级推理数据、过程奖励模型（PRM）和强化学习训练，支持细粒度奖励的推理时间缩放。

Result: 在具有挑战性的视觉语言基准测试中取得了持续改进的强基线结果，并通过消融研究揭示了各组件的影响和推理时间缩放的有趣特性。

Conclusion: 该工作为视觉语言模型建立了基准，并为更复杂的多模态推理提供了见解，相关数据集、PRM和代码将开源。
...
<details>
  <summary>Details</summary>
Motivation: 解决食物图像分割需要大量像素级标注的问题，利用弱监督学习降低标注成本，为食物营养追踪等应用提供支持
...
TL;DR: DyL-UNet是一种基于动态学习的超声心动图分割架构，通过构建回声动态图和心脏相位动态注意力机制，在保持分割精度的同时显著提升时间一致性。


<details>
  <summary>Details</summary>
Motivation: 超声心动图容易受到变形和斑点噪声影响，导致帧间分割结果不稳定，即使单帧分割精度高，时间不稳定性也会削弱功能评估和临床可解释性。

Method: 提出DyL-UNet架构，通过动态学习构建回声动态图提取视频动态信息，采用多个Swin-Transformer编码器-解码器分支处理单帧图像，在跳跃连接处引入心脏相位动态注意力机制。

Result: 在CAMUS和EchoNet-Dynamic数据集上的实验表明，DyL-UNet在保持与现有方法相当分割精度的同时，实现了更优的时间一致性。

Conclusion: DyL-UNet为自动化临床超声心动图分析提供了可靠的解决方案，能够实现时间稳定且精确的分割。
...
<details>
  <summary>Details</summary>
Motivation: 评估VLMs在复杂视觉模式中识别数字信息的能力，特别是在对抗性环境下，以发现模型在现实应用中的局限性。

Method: 创建包含500张类似石原色盲测试图像的数据集，包含0-99数字的不同颜色组合，使用是/否和开放式提示评估9个VLMs，并与人类参与者进行对比。

Result: 实验显示VLMs在对抗性环境中识别数字的能力存在局限，普遍存在幻觉问题，表现不如人类参与者。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在稀疏视图设置下性能急剧下降，现有方法使用扩散模型修复损坏的渲染图作为伪真值进行优化，但计算成本高昂。

Method: 提出小波域扩散方法：在低分辨率LL子带应用扩散，高频子带使用轻量网络优化；采用高效的在线随机掩码策略替代低效的留一法来构建训练对。
...
TL;DR: Sa2VA-i是对Sa2VA模型的改进版本，通过修正训练与推理过程中的不一致性问题，在多个视频分割基准上实现了新的SOTA性能
...
Method: 提出Sa2VA-i模型，通过修正训练与推理过程中的不一致性问题来改进原Sa2VA模型

Result: 在多个视频基准上取得显著提升：MeViS +11.6 J&F，Ref-YT-VOS +1.4，Ref-DAVIS +3.3，ReVOS +4.1。Sa2VA-i-1B模型在MeViS基准上甚至能与原Sa2VA-26B模型相媲美
...
TL;DR: 提出一种无需训练的方法，将多光谱数据作为零样本输入引入到仅训练于RGB数据的通用多模态模型中，使模型能够理解专业的多光谱信号。


<details>
  <summary>Details</summary>
Motivation: 多光谱图像在遥感应用中很重要，但现有机器学习模型需要专门训练且成本高。通用多模态模型虽然强大，但无法理解专业的多光谱输入。

Method: 利用多模态模型对视觉空间的理解，将多光谱数据适配到该空间，并将领域特定信息作为指令注入模型，以零样本方式工作。

Result: 在Gemini2.5模型上测试，在土地覆盖和土地利用分类的遥感基准测试中获得了显著的零样本性能提升。

Conclusion: 该方法展示了地理空间专业人员可以轻松利用强大多模态模型处理专业传感器数据，加速工作流程。
...
Method: 提出新颖的多模态训练方法，集成检测、分割和多模态链式推理，发布涵盖推理、检测、分割和文档理解任务的开源数据集。

Result: 评估显示Citrus-V在多个基准测试中优于现有开源医学模型和专家级影像系统，支持精确病变量化、自动报告生成和可靠第二意见。

Conclusion: Citrus-V提供了从视觉定位到临床推理的统一流程，为临床诊断、治疗规划和手术决策提供有力支持。
...
TL;DR: 该研究评估了多模态大语言模型在交通事故检测中的零样本能力，使用基础设施摄像头图像，通过集成先进视觉分析技术提升模型性能。Pixtral模型表现最佳，F1分数0.71，召回率83%。


<details>
  <summary>Details</summary>
Motivation: 交通事故检测对公共安全至关重要，但缺乏多样化的标注数据集限制了传统方法的可扩展性。研究旨在探索MLLMs在无需大量标注数据的情况下进行交通事故检测的潜力。
...
Result: Pixtral表现最佳（F1=0.71，召回率83%），Gemini模型通过增强提示精度提升至90%但F1和召回率下降，Gemma 3性能最平衡。集成视觉分析技术显著提升了模型性能。

Conclusion: MLLMs与先进视觉分析技术结合在自动交通监控系统中具有巨大应用潜力，能够实现高效的事故检测而无需大量标注数据。
...
<details>
  <summary>Details</summary>
Motivation: 解决长期点跟踪问题，需要在视频帧间实现一致的点识别，特别是在外观变化、运动和遮挡的情况下。针对在线设置，适用于实时和流式应用。

Method: 扩展Track-On模型为Track-On2，采用因果处理框架，通过内存机制保持时间一致性，无需未来帧信息。推理时进行粗粒度块级分类后细化。系统研究合成训练设置对内存行为的影响。

Result: 在五个合成和真实世界基准测试中取得最先进结果，超越先前的在线跟踪器甚至利用双向上下文的强离线方法。

Conclusion: 基于因果、内存架构且纯合成数据训练的模型是解决现实世界点跟踪问题的可扩展有效方案。
...
TL;DR: KAMERA是一个用于多相机、多光谱同步和实时检测海豹与北极熊的综合系统，在阿拉斯加海域的空中调查中，将数据集处理时间减少了80%


<details>
  <summary>Details</summary>
Motivation: 为冰相关海豹的空中调查提供高效的多光谱检测系统，解决传统方法处理时间长的问题

Method: 采用严格的校准和硬件同步技术，使用多光谱进行目标检测，所有收集的数据都带有元数据注释，便于后续参考

Result: 系统实现了80%的数据处理时间减少，所有图像和动物检测结果都能映射到世界平面上，提供准确的调查区域估计
...
TL;DR: NeuCODEX是一种神经形态协同推理架构，通过联合优化空间和时间冗余，显著减少数据传输和边缘能耗，实现高效的SNN边缘部署。
...
Method: 引入学习驱动的脉冲压缩模块减少数据传输，采用动态提前退出机制基于输出置信度自适应终止推理，在真实边缘-云测试平台上基于ResNet-18和VGG-16骨干网络进行原型验证。

Result: 数据传输减少高达2048倍，边缘能耗降低超过90%，端到端延迟相比仅边缘推理减少3倍，准确率下降小于2%。
...
TL;DR: 本文提出了一种鲁棒的自监督立体匹配方法，通过引入视觉基础模型的先验知识和场景对应先验，解决了恶劣天气条件下立体匹配性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督立体匹配方法在恶劣天气条件下性能显著下降，主要原因是CNN特征提取器在退化区域表现不佳，以及基于光度一致性假设的监督信号失效。

Method: 提出从视觉基础模型注入鲁棒先验到CNN特征提取器，并构建场景对应先验作为监督信号。通过创建包含清晰和恶劣天气图像对的合成数据集，设计了两阶段训练范式：鲁棒自监督场景对应学习和恶劣天气知识蒸馏。

Result: 大量实验表明，该方法在恶劣天气条件下显著优于现有的最先进自监督方法，证明了其有效性和通用性。

Conclusion: 通过引入视觉基础模型的先验知识和场景对应先验，成功提升了立体匹配模型在恶劣天气条件下的鲁棒性，为自监督立体匹配在真实世界应用提供了有效解决方案。
...
TL;DR: 提出了基于YOLO的息肉检测框架YOLO-LAN，通过M2IoU损失函数、数据增强和负样本训练，在结肠镜息肉检测任务中实现了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 结肠癌始于结肠内壁的息肉病变，结肠镜检查是标准检测方法，但人工检测存在不一致性和漏检问题，需要基于深度学习的实时准确检测方案。

Method: 开发了YOLO-LAN检测框架，采用M2IoU损失函数、多样化数据增强技术和负样本训练来模拟真实临床场景，使用YOLOv12和YOLOv8两种架构。
...
TL;DR: 该技术报告分析了LSVOS挑战赛中MOSEv2赛道，通过改进SeC（增强版SAM-2框架），研究了长期记忆和概念感知记忆机制，在测试集上获得39.89%的JF分数，排名第一。


<details>
  <summary>Details</summary>
Motivation: 探索复杂半监督视频对象分割问题，特别是针对MOSEv2赛道中的核心挑战，如遮挡和重现、干扰物抑制等。

Method: 分析和改进SeC框架，重点研究其长期记忆机制（保持时间连续性）和概念感知记忆（提供语义先验），这两种机制协同工作解决视频分割中的关键问题。

Result: 在MOSEv2测试集上获得39.89%的JF分数，在LSVOS挑战赛中排名第一。

Conclusion: 长期记忆和概念感知记忆的结合有效提升了视频对象分割性能，特别是在处理遮挡、重现和干扰物等挑战性场景时表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLMs通过序列化图像处理视觉信息，与人类视觉的并行处理方式存在差异，且其不透明的内部机制阻碍了深入理解和架构创新。受人类视觉双流假说启发，希望深入理解VLMs的视觉处理机制。

Method: 1）将图像转换为文本标记图研究物体识别的两阶段过程；2）理论推导和实证验证VLMs位置表示的几何结构；3）提出基于即插即用视觉解码器的指令无关标记压缩算法和RoPE缩放技术。

Result: 验证了VLMs视觉处理的分析结果，物体识别呈现从浅层到深层的两阶段过程，空间感知具有特定的几何结构。提出的方法有效提升了解码效率和空间推理能力。

Conclusion: 该研究为理解VLM内部机制提供了深入见解，并为设计更强大的未来架构提供了明确原则，推动了VLMs的进一步发展。
...
TL;DR: 本文提出了一种无视觉编码器的文本到文本检索方法，通过使用VLLM生成结构化图像描述来替代传统文本到图像检索，显著减少了模态差距并提高了组合性。
...
Method: 采用无视觉编码器的单编码器检索管道，将文本到图像检索迁移到文本到文本范式，利用VLLM生成结构化图像描述。

Result: 该方法在多个检索和组合性基准测试中达到最先进的零样本性能，仅需少量GPU小时进行校准，且模型参数小至0.3B。
...
Result: 发现组合性训练能提高长标题检索性能，长标题训练也能促进组合性，但这些增益对数据质量和模型设计敏感。高质量长标题数据训练能在两个任务上都取得强性能。
...
TL;DR: 提出一个利用合成RGB图像、少量真实标注和GAN跨模态对齐的框架，以提升热图像中的植物语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 热图像中植物分割在户外高通量表型分析中面临挑战，主要是植物与杂草的低对比度和频繁遮挡问题。
...
Conclusion: 合成数据与有限手动标注及生成模型的跨域翻译相结合，能显著提升复杂田间环境中多模态图像的语义分割性能。
...
TL;DR: 提出了HyKid数据集，包含48名儿童脑积水患者的3D MRI图像和手动校正的脑组织分割，特别是脉络丛分割，为脑积水评估提供了高质量基准。
...
Method: 使用48名儿童脑积水患者的3D MRI图像，通过切片到体积算法从常规低分辨率图像重建1mm各向同性分辨率图像，由经验丰富的神经学家手动校正脑组织分割，并使用检索增强生成框架从临床放射学报告中提取结构化数据。
...
Method: MsFIN包含三个层次：多尺度特征聚合层使用多尺度模块提取短期、中期和长期时间尺度的场景表示，并利用Transformer架构促进全面特征交互；时序特征处理层在因果约束下捕捉场景和对象特征的序列演化；多尺度特征后融合层融合多个时间尺度的场景和对象特征生成综合风险表示。
...
TL;DR: 本文提出了一种基于持续学习的面部伪造检测方法，通过开发性混合专家架构（MoE）来解决伪造技术快速演变带来的检测挑战。该方法使用LoRA模型作为专家，分为Real-LoRA学习真实面部特征和多个Fake-LoRA捕获不同伪造类型的信息，有效防止灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 随着数字面部生成和操纵技术的快速发展，现有检测模型难以跟上不断演变的伪造技术。需要一种能够快速适应新伪造类型、同时避免遗忘已学知识的检测方法。

Method: 采用开发性混合专家架构，使用LoRA模型作为专家。Real-LoRA学习真实面部特征，多个Fake-LoRA捕获增量伪造信息。通过正交梯度和正交损失防止梯度干扰和灾难性遗忘。

Result: 在数据集和操纵类型增量协议下的实验结果表明该方法有效，能够持续学习新伪造类型而不遗忘先前知识。

Conclusion: 该方法成功将面部伪造检测构建为持续学习问题，通过创新的MoE架构和正交约束机制，实现了对不断演变的伪造技术的有效防御。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态扩散语言模型仅支持简单的图像级理解任务和低分辨率图像生成，缺乏高级功能如物体定位和高质量图像编辑。Lavida-O旨在填补这一空白。
...
Conclusion: Lavida-O是首个统一的掩码扩散模型，成功将图像理解能力用于提升生成和编辑质量，在多模态任务上实现了突破性进展。
...
TL;DR: 本文提出了基于概念的视频相似度估计任务（ConViS），通过预定义的关键语义概念来计算可解释的视频相似度分数，并建立了ConViS-Bench基准数据集来评估模型性能。
...
Method: 提出ConViS任务框架，使用预定义的语义概念来分解视频相似度评估，并构建ConViS-Bench基准数据集，包含精心标注的视频对和概念级相似度分数。

Result: 在ConViS基准上测试了多个先进模型，结果显示不同概念对模型构成不同程度的挑战，某些概念的视频相似度估计更具难度。

Conclusion: ConViS-Bench将成为语言驱动视频理解研究的重要资源，有助于推动模型在细粒度视频比较任务上的发展。
...
<details>
  <summary>Details</summary>
Motivation: 连续人体运动理解在计算机视觉中面临高维度和固有冗余的挑战，需要高效的压缩和表示方法来分析复杂的运动动态。

Method: 采用密集运动标记化与对抗性精炼相结合的VQ-GAN框架，消除重建伪影如运动模糊和时间不对齐问题。

Result: 在CMU Panoptic数据集上，方法比dVAE基线在SSIM指标上提升9.31%，时间不稳定性降低37.1%。研究发现2D运动可用128个标记的紧凑词汇表表示，而3D运动需要1024个标记的更大码本。
...
TL;DR: 该论文提出了一种新的图放射组学学习（GrRAiL）描述符，用于在临床MRI扫描中表征病变内异质性，通过图论方法量化空间关联，在区分肿瘤复发与放射效应方面显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决在实体肿瘤中可靠区分恶性病变与混淆病理的挑战，传统放射组学方法会丢失复杂的空间关系信息。

Method: GrRAiL方法首先使用逐体素放射组学测量识别亚区域簇，然后计算图论指标来量化簇间的空间关联，生成加权图编码高阶空间关系。

Result: 在947名受试者的多中心评估中，GrRAiL在三个应用场景中均显著优于现有方法：胶质母细胞瘤（测试准确率78%，提升>10%）、脑转移瘤（74%，提升>13%）和胰腺IPMN风险分层（75%，提升>10%）。

Conclusion: GrRAiL能够可靠地捕获病变内异质性，有效区分恶性病变与混淆病理，在临床应用中显示出优越性能。
...
TL;DR: CLOPS是第一个仅使用以自我为中心的视觉来感知环境和导航的人类化身，通过将低级运动技能学习与高级视觉控制解耦，实现了更人类化的运动生成。


<details>
  <summary>Details</summary>
Motivation: 当前的人类运动生成方法忽视了感知与运动之间的相互依赖关系，使用的感知方式与人类感知差异很大。作者认为生成人类化化身行为需要人类化的感知方式。

Method: 首先在大型运动捕捉数据集上训练运动先验模型，然后使用Q学习训练策略，将以自我为中心的视觉输入映射到运动先验的高级控制命令。

Result: 实验证明以自我为中心的视觉能够使化身产生人类化的运动特征，例如避开视觉场中的障碍物。

Conclusion: 为化身配备人类化传感器，特别是以自我为中心的视觉，有望训练出行为更像人类的化身。
...
TL;DR: 该论文针对布局到图像生成中边界框重叠问题，提出了OverLayScore指标和OverLayBench基准，并开发了CreatiLayout-AM模型来改善重叠场景下的生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前布局到图像生成方法在处理边界框显著重叠的布局时表现不佳，现有基准存在偏向简单案例的偏差，无法有效评估模型在挑战性重叠条件下的性能。

Method: 引入OverLayScore量化重叠边界框复杂度，构建OverLayBench基准提供高质量标注和平衡分布，提出CreatiLayout-AM模型在精选的amodal掩码数据集上进行微调。

Result: 分析显示现有基准偏向低OverLayScore的简单案例，新基准和模型为处理复杂重叠场景提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于学习的3D重建方法依赖于真实世界多视图数据，但这些数据并不总是可用。视频扩散模型具有强大的想象力，但其2D特性限制了在机器人导航等需要3D交互的应用

Method: 在典型的RGB解码器基础上增加3D高斯溅射解码器，通过RGB解码器的输出进行监督训练。3DGS解码器可以完全使用视频扩散模型生成的合成数据进行训练

Result: 实验结果表明，该框架在静态和动态3D场景生成方面达到了最先进的性能

Conclusion: 该框架能够从文本提示或单张图像实时合成3D场景，并扩展到从单目输入视频生成动态3D场景，为物理AI应用提供了有效的3D环境生成解决方案
...
TL;DR: VolSplat提出了一种新的多视角前馈3D高斯溅射范式，用体素对齐的高斯分布取代像素对齐方法，解决了现有方法对输入视角数量的依赖、视角偏差密度分布和对齐误差等问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于像素对齐高斯预测的方法存在多个固有局限性：重建的3D模型严重依赖输入视角数量，导致视角偏差密度分布，在源视角存在遮挡或低纹理时引入对齐误差。

Method: VolSplat直接从预测的3D体素网格预测高斯分布，避免了像素对齐对易出错的2D特征匹配的依赖，确保鲁棒的多视角一致性，并能基于3D场景复杂度自适应控制高斯密度。

Result: 在RealEstate10K和ScanNet等基准测试中，VolSplat实现了最先进的性能，产生更合理和视角一致的高斯重建，渲染质量得到提升。

Conclusion: 该方法不仅获得优越结果，还为前馈3D重建建立了更可扩展的框架，提供了更密集和鲁棒的表示，为更广泛社区的研究铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 现有的扩散和流匹配方法需要模型同时学习质量传输和条件注入，这增加了模型的学习负担。为了减轻这种负担，作者提出了条件感知重参数化方法。

Method: CAR-Flow通过轻量级的学习偏移来调整源分布、目标分布或两者，从而缩短模型需要学习的概率路径。该方法可以集成到现有的流匹配框架中。

Result: 在低维合成数据上，CAR-Flow可视化并量化了效果。在ImageNet-256数据集上，将CAR-Flow集成到SiT-XL/2模型中，FID从2.07降至1.68，仅增加不到0.6%的参数。
