...
TL;DR: 提出了一种轻量级测试时增强方法CI-TTA，通过弹性变形和网格变形生成输入图像的多个变体，并通过置信度引导的过滤机制聚合预测结果，在域泛化任务中取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 深度模型在分布偏移下性能显著下降，现有方法大多依赖多域训练或计算密集的测试时适应，需要一种轻量且互补的策略来提升域泛化能力

Method: 开发了类不变测试时增强技术(CI-TTA)，通过弹性变形和网格变形生成保持类别不变的图像变体，采用置信度引导的过滤方案去除不可靠输出，确保最终决策基于一致且可信的线索

Result: 在PACS和Office-Home数据集上的广泛实验表明，该方法在不同域泛化算法和骨干网络上均取得一致性能增益

Conclusion: CI-TTA是一种有效且通用的方法，能够显著提升模型在未见域上的泛化性能，且计算开销小
...
TL;DR: AToken是首个统一的视觉分词器，能在图像、视频和3D资产上同时实现高保真重建和语义理解，通过4D潜在空间统一多种模态和任务。


<details>
  <summary>Details</summary>
Motivation: 现有分词器通常专注于单一模态的重建或理解任务，缺乏一个统一的框架来处理多种视觉输入模态。AToken旨在解决这一限制，为下一代多模态AI系统提供统一的视觉分词基础。

Method: 采用纯Transformer架构，使用4D旋转位置编码处理任意分辨率和时长的视觉输入。引入无对抗训练目标，结合感知损失和Gram矩阵损失确保稳定训练。采用渐进式训练课程，从单图像扩展到视频和3D，支持连续和离散潜在token。

Result: 在图像上达到0.21 rFID和82.2% ImageNet准确率；视频上达到3.01 rFVD和32.6% MSRVTT检索率；3D上达到28.19 PSNR和90.9%分类准确率。在下游任务中支持视觉生成和理解任务，在所有基准测试中表现竞争力。

Conclusion: AToken展示了统一视觉分词在构建下一代多模态AI系统中的潜力，为同时处理重建和理解任务提供了有效的统一框架。
...
Method: 1) 海马体启发的视图对齐模块：通过连续表示中的结构对齐来捕获新视图的增益信息；2) 认知遗忘机制：模拟人类记忆衰减模式来调节历史知识权重；3) 前额叶皮层启发的知识巩固模块：利用时间张量稳定性逐步巩固历史知识。
...
Conclusion: 提出的边缘条件调制方法为推进感知超分辨率提供了参数高效的路径，实现了稳定的对抗精炼和增强的边缘保真度，无需更深或过度参数化的架构。
...
TL;DR: 提出基于分数扩散模型的自适应迭代点云去噪方法，通过估计噪声变化确定自适应去噪计划，使用两阶段采样策略实现特征和梯度融合，在合成和真实扫描数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法需要经验性地重复去噪过程来处理不同噪声水平，缺乏高效安排迭代去噪过程的方法来处理不同模式或水平的噪声。

Method: 基于分数扩散模型的自适应迭代去噪：1）估计噪声变化并确定自适应去噪计划；2）设计网络架构和两阶段采样策略实现特征融合和梯度融合；3）迭代调用训练网络更新点云。

Result: 方法获得干净平滑的去噪点云，更好地保持形状边界和细节，在合成数据集（不同噪声模式）和真实扫描数据集上均优于现有方法，定性和定量结果都更好。

Conclusion: 提出的自适应迭代点云去噪方法通过分数扩散模型和自适应调度，有效处理不同噪声模式，在保持形状细节的同时获得优质去噪效果，优于当前最先进方法。
...
Method: 提出DiffVL框架，利用扩散模型将噪声GPS轨迹与视觉BEV特征和标准地图条件结合，通过迭代扩散细化恢复真实位姿分布，学习逆转GPS噪声扰动。
...
TL;DR: DICE是一个将扩散模型与共识均衡框架结合的稀疏视图CT重建方法，通过数据一致性代理和先验代理的迭代平衡，在15、30、60视图的稀疏采样下显著优于现有方法
...
Method: 提出Diffusion Consensus Equilibrium (DICE)框架，将双代理共识均衡集成到扩散模型采样过程中：1)数据一致性代理通过近端算子强制测量一致性；2)先验代理通过扩散模型在每个采样步骤执行干净图像估计

Result: 实验结果显示DICE在15、30、60视图（总共180视图）的均匀和非均匀稀疏视图设置下，显著优于最先进的基线方法，重建出高质量CT图像
...
Method: 利用常规记录的patient-level诊断结果作为弱监督，通过Shared Aggregation Tokens和Max-Severity Triplet Loss来对齐跨域类别分布，利用患者级诊断由最严重区域决定的特点。

Result: 实验结果表明该方法优于比较的域适应方法，在存在域偏移的情况下提高了溃疡性结肠炎严重程度估计的性能。

Conclusion: 提出的弱监督域适应方法有效解决了目标域标注稀缺的问题，利用常规可得的患者级诊断信息显著改善了跨域溃疡性结肠炎严重程度估计的效果。
...
Method: 使用100张图像（50张真实照片+50张合成场景），12名参与者提供230份标注，涵盖30个维度。评估7个VLMs的零样本性能，使用结构化提示和确定性解析器。

Result: 模型在可见客观属性上表现优于主观评价，最佳系统（claude-sonnet）在多标签项目上达到宏观0.31和平均Jaccard 0.48。人类一致性高的维度模型得分也更高，合成图像略微降低得分。

Conclusion: 研究发布了可复现的基准测试、提示词和评估工具，支持参与式城市分析中的不确定性感知评估，表明VLMs在城市感知任务中具有潜力但仍有改进空间。
...
TL;DR: 提出了一种用于动态点云压缩的特征对齐运动变换(FMT)框架，通过隐式建模时间变化和潜在空间条件编码，取代显式运动估计，显著提升了压缩效率和编解码性能。
...
Result: 实验表明该方法在编码和解码效率上均优于D-DPCC和AdaDPCC，分别实现了20%和9.4%的BD-Rate降低，同时支持帧级并行压缩。
...
Method: 提出了HybridMamba架构，包含两个互补机制：1）特征扫描策略，通过轴向遍历和局部自适应路径逐步整合表示；2）结合空间-频率分析的门控模块，用于全面的上下文建模。
...
TL;DR: 该论文提出了一种动态跳跃连接（DSC）块来解决传统U型网络中跳跃连接的两个关键限制：特征间约束和特征内约束，通过测试时训练模块和动态多尺度核模块实现自适应特征融合。


<details>
  <summary>Details</summary>
Motivation: 传统U型网络中的跳跃连接存在两个主要问题：特征间约束（固定路径传输信息，缺乏内容感知）和特征内约束（多尺度特征交互建模不足，全局上下文信息聚合效果差）。

Method: 提出动态跳跃连接（DSC）块，包含两个组件：1）测试时训练（TTT）模块，在推理时动态调整隐藏表示，实现内容感知特征精炼；2）动态多尺度核（DMSK）模块，根据全局上下文线索自适应选择核大小，增强多尺度特征集成能力。

Result: DSC块具有架构无关性，可无缝集成到现有的基于CNN、Transformer、混合CNN-Transformer和Mamba的U型网络中，实验证明了其即插即用的有效性。

Conclusion: 动态跳跃连接块通过自适应机制显著增强了跨层连接性，有效解决了传统跳跃连接的局限性，为医学图像分割提供了更强大的特征融合方案。
...
TL;DR: LSTC-MDA框架通过长短期时间卷积模块和联合混合数据增强，解决了骨架动作识别中标注数据稀缺和时空依赖建模的挑战，在多个基准数据集上达到SOTA性能。
...
Method: 提出统一框架LSTC-MDA，包含：1）LSTC模块-并行短长期分支的时间卷积，通过相似度权重自适应对齐融合特征；2）扩展JMDA数据增强-输入级加法混合，限制相同相机视角内的混合以避免分布偏移。

Result: 在多个基准数据集上达到最优性能：NTU 60（X-Sub 94.1%，X-View 97.5%）、NTU 120（X-Sub 90.4%，X-Set 92.0%）、NW-UCLA 97.2%。消融实验证实各组件有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于指令的图像编辑方法在处理挑战性编辑任务时表现不佳，因为现有数据集的编辑类型和样本数量有限，且传统数据集构建方法包含噪声图像-标题对，可能在复杂编辑场景中引入偏见并限制模型能力。

Method: 采用新颖的数据集构建流水线，利用两个多模态大语言模型（MLLMs）分别生成视觉自适应编辑指令和生成高保真编辑图像，构建包含18种非风格迁移编辑类型和38种风格迁移操作的多样化数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉解释生成方法缺乏适应性，无法应用于复杂模型，需要一种既能生成解释又能提升模型可解释性的新方法。

Method: 提出ALA机制自动选择注意力层，无需人工干预；AEA机制每隔一个epoch更新ALA参数，解决注意力区域过小问题。结合两种机制实现解释生成和模型参数更新。

Result: 在CUB-200-2011和ImageNet-S数据集上，在mean IoU、插入分数、删除分数和插入-删除分数等指标上均优于基线方法，CUB-200-2011数据集上mean IoU提升53.2个百分点。
...
TL;DR: DACoN是一个利用基础模型进行线条动画自动上色的框架，通过融合语义特征和空间特征来解决遮挡、姿态变化等挑战，支持任意数量参考图像。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法在动画线条图上色时难以处理遮挡、姿态变化和视角变化等问题，需要更鲁棒的特征提取方法。

Method: 提出DACoN框架，融合基础模型的低分辨率语义特征和CNN的高分辨率空间特征，无需Multiplex Transformer限制，支持任意数量参考图像。

Result: 定量和定性评估表明，使用多个参考图像能获得更好的上色性能，优于现有方法。

Conclusion: DACoN通过语义-空间特征融合和多参考图像支持，显著提升了动画线条图自动上色的准确性和鲁棒性。
...
TL;DR: FMGS-Avatar：一种从单目视频重建高保真可动画人体化身的创新方法，通过网格引导的2D高斯泼溅和基础模型知识蒸馏，解决了现有方法在表面细节保持和信息不足方面的挑战。
...
Method: 提出FMGS-Avatar方法，包含两个关键创新：1）网格引导的2D高斯泼溅，将2D高斯基元直接附加到模板网格面上，约束位置、旋转和移动；2）利用在大规模数据集上训练的基础模型（如Sapiens）补充单目视频的有限视觉线索，并通过选择性梯度隔离的协调训练策略解决多模态先验知识蒸馏中的优化目标冲突。

Result: 实验评估显示，与现有方法相比，该方法在重建质量方面表现优越，在几何精度和外观保真度方面有显著提升，同时提供丰富的语义信息。蒸馏的先验知识在共享规范空间中自然实现了新颖视角和姿态下空间和时间一致的渲染。

Conclusion: 通过增强的表示和协调的信息蒸馏相结合，FMGS-Avatar显著推进了3D单目人体化身重建技术，为解决单视图观察中的几何信息不足问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法仅将MLLM用于评估而非排序过程，未能充分利用其多模态推理能力，导致检索性能不佳

Method: 设计列表排序提示使MLLM直接参与候选图像重排序，基于图像评估提示分析候选与查询的匹配度，并引入查询解构提示进行细粒度语义分析

Result: 在五个数据集上的实验表明，CoTRR在文本到图像检索、组合图像检索和聊天式图像检索三个任务中均达到最先进性能
...
TL;DR: 利用扩散模型生成合成数据来改善历史艺术品中气味相关物体的检测性能，特别是在标注稀缺的细分领域


<details>
  <summary>Details</summary>
Motivation: 历史艺术品中气味参考检测面临标注类别精细、标注稀疏和极端类别不平衡等挑战，传统方法难以获得足够标注数据

Method: 评估多种基于扩散模型的数据增强策略，将合成数据整合到模型训练中，利用扩散模型的大规模预训练能力

Result: 合成数据的加入提高了检测性能，该方法在数据量较少时仍然有效，且具有进一步扩展的潜力

Conclusion: 扩散模型为基础的合成数据生成为解决标注稀缺的细分领域检测问题提供了有前景的途径
...
TL;DR: 提出了首个帧级准确的小型视觉语言模型视频问答基准测试，在受控帧采样策略下评估模型性能，确认了现有基准存在帧采样偏差问题


<details>
  <summary>Details</summary>
Motivation: 当前视频基准测试存在显著的帧采样偏差，不同模型使用不同的帧选择策略进行评估，导致性能比较不公平且不可靠

Method: 创建帧级准确的基准测试，在严格控制的不同帧采样策略下评估最先进的小型视觉语言模型，并开源基准测试代码

Result: 结果证实了怀疑的偏差存在，并揭示了不同帧采样技术下SVLMs的数据特定和任务特定行为

Conclusion: 需要为每个基准数据集定制标准化的帧采样策略，提供了可重现且无偏的视频VLM评估协议
...
TL;DR: 提出了一种多模型参数化表示方法，通过高斯混合模型分割点云，平面簇拟合为平面，曲面簇用B样条拟合，实现实时表面检测和拟合，在效率和精度上均有显著提升


<details>
  <summary>Details</summary>
Motivation: 现有参数化点云表示方法中，自适应模型（如样条曲面）计算成本高，而实时方法（如高斯混合模型）自由度低难以用少量基元实现高精度

Method: 首先用高斯混合模型分割点云为多个簇，选择平坦簇合并为平面或曲面。平面用2D体素边界描述方法拟合和界定，曲面用B样条曲面拟合并采用相同边界描述方法

Result: 在多个公开数据集上评估，表面检测比最先进方法鲁棒性更强，效率提升3.78倍，精度比高斯混合模型提高2倍，在低功耗机载计算机上达到36.4 fps

Conclusion: 该方法实现了实时高效的表面检测和拟合，在保持高精度的同时显著提升了计算效率，适用于内存高效建图和多机器人协作等应用
...
TL;DR: Report2CT是一个基于放射学报告的3D CT生成框架，使用多文本编码器和潜在扩散模型，直接从完整的放射报告中生成高质量的胸部CT体积，在MICCAI 2025 VLM3D挑战赛中取得第一名。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像生成方法使用简化的提示，忽略了放射报告中丰富的语义细节，导致文本-图像对齐和临床保真度降低。需要开发能够利用完整放射报告细节的3D CT生成方法。

Method: 提出Report2CT框架，集成三个预训练的医学文本编码器（BiomedVLP CXR BERT、MedEmbed和ClinicalBERT）来捕捉临床上下文。使用放射报告和体素间距信息作为条件，在CT RATE数据集的20000个CT体积上训练3D潜在扩散模型。

Result: 生成的CT体积具有解剖一致性和优秀的视觉质量，文本-图像对齐效果显著。多编码器条件提高了CLIP分数，分类器自由引导进一步增强了对齐效果。在MICCAI 2025 VLM3D挑战赛中排名第一，在所有评估指标上达到最先进性能。

Conclusion: 通过利用完整放射报告和多编码器文本条件，Report2CT推进了3D CT合成技术，能够生成临床保真度高、质量优秀的合成数据。
...
TL;DR: 提出了一种针对骨分割的骨折交互式测地线活动轮廓算法，通过结合强度与梯度范数的新型边缘检测函数和嵌入骨折提示的距离信息，解决了传统方法在边缘阻塞、边缘泄漏和骨折现象中的局限性。
...
Method: 设计最小能量变形(MED)损失函数作为正则化器，与广泛使用的Chamfer距离相结合，并将其整合到V2C-Flow模型中

Result: 在不损害重建精度和拓扑正确性的前提下，显著改善了之前被忽视的训练一致性和可重复性

Conclusion: MED损失函数有效解决了皮层表面重建中变形能量优化和训练一致性的挑战，为神经影像分析提供了更可靠的皮层重建方法
...
<details>
  <summary>Details</summary>
Motivation: 现有骨健康AI研究主要依赖视觉数据且缺乏可解释性，而医疗应用需要透明可信的决策过程，特别是在欧盟AI法案背景下

Method: 提出基于原型的多模态架构，同时使用腰椎DEXA扫描和患者记录，设计上就具备可解释性

Result: 在4,160名NHS患者数据集上，视觉单模态准确率87.58%，多模态准确率89.8%，均超越现有方法

Conclusion: ProtoMedX在保持高性能的同时提供临床可理解的解释，满足医疗AI对准确性和可解释性的双重需求
...
TL;DR: MapAnything是一个自动从单张图像计算物体地理坐标的模块，利用度量深度估计模型、几何原理和相机参数来估算物体距离和位置，并在城市环境中验证了其准确性。


<details>
  <summary>Details</summary>
Motivation: 随着城市数字化程度提高，需要大量人工维护城市物体数据库（如交通标志、树木、事件等），手动更新地理坐标数据成本高昂且效率低下，需要自动化解决方案。

Method: 使用先进的度量深度估计模型，结合物体到相机的距离计算、几何原理和相机规格参数，自动确定物体的地理坐标。通过LiDAR点云数据在城市环境中验证距离估计的准确性。

Result: 评估显示模块在不同距离区间和语义区域（如道路、植被）中都能有效工作，通过交通标志和道路损坏的实际用例证明了其有效性。

Conclusion: MapAnything模块为城市物体和事件的地理坐标自动映射提供了可行的自动化解决方案，能够显著减少人工工作量并保持数据库的实时更新。
...
TL;DR: 本文发现超分辨率模型主要对噪声过拟合而非所有退化类型，提出针对性特征去噪框架，包含噪声检测和去噪模块，无需修改模型架构即可提升泛化性能。
...
Method: 提出针对性特征去噪框架，包含噪声检测模块和去噪模块，可以无缝集成到现有超分辨率模型中，无需架构修改。

Result: 在五个传统基准测试和数据集（包括合成和真实场景）上，相比之前的基于正则化的方法，展现了更优越的性能。
...
TL;DR: 本工作复现了Faithful Vision Transformers (FViTs)的结果，研究了扩散去噪平滑(DDS)在提升Vision Transformers可解释性鲁棒性方面的有效性，并扩展了原始研究，验证了DDS对各种可解释性方法的普适性改进。


<details>
  <summary>Details</summary>
Motivation: 验证arXiv:2311.17983论文中关于扩散去噪平滑(DDS)能够提升Vision Transformers在分割任务和分类任务中对抗攻击鲁棒性的主张，并扩展研究DDS对不同可解释性方法的通用改进效果。

Method: 复现FViTs方法，使用Diffusion Denoised Smoothing (DDS)技术，在分割任务和分类任务中测试对抗攻击鲁棒性，并扩展到基线方法和Attribution Rollout方法，同时评估计算成本和环境影响。

Result: 研究结果总体上支持原始研究的发现，DDS确实能够提升Vision Transformers的可解释性鲁棒性，但在某些细节上存在微小差异。同时验证了DDS对不同可解释性方法的普适性改进效果。

Conclusion: 扩散去噪平滑(DDS)是一种有效的技术，能够提升Vision Transformers在各种任务中的可解释性鲁棒性，且具有较好的通用性，但需要关注其计算成本和环境影响。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像分类方法需要大量参数训练和标注数据，而现有的视觉语言模型依赖单次表示，往往无法捕捉视觉内容的互补方面。

Method: 使用多智能体框架：Outliner Agent分析图像全局主题并生成提示，三个Aspect Agent从不同视觉维度提取细粒度描述，Reasoning Agent通过集成反思步骤综合这些输出进行统一分类。

Result: 在4个不同的图像分类基准数据集上，MARIC显著优于基线方法。

Conclusion: 多智能体视觉推理为鲁棒和可解释的图像分类提供了有效方法，缓解了参数密集型训练和单一VLM推理的缺点。
...
<details>
  <summary>Details</summary>
Motivation: 随着人像图像在计算机视觉中的广泛应用，需要在保护个人身份的同时保持图像对下游任务的可用性

Method: 利用潜在扩散模型的修复能力，设计自适应属性引导模块在反向去噪过程中应用梯度校正，使生成图像的面部属性与合成目标图像对齐

Result: 在CelebA-HQ和FFHQ数据集上的实验表明，该方法优于现有最先进方法且无需额外模型训练

Conclusion: 该框架提供了可控的人像匿名化解决方案，在保护隐私的同时保持了图像实用性
...
TL;DR: 该研究提出了一种从乳腺癌患者治疗早期MRI影像中学习动态表征的方法，用于预测新辅助化疗后的病理完全缓解(pCR)。通过多任务学习模型在潜在空间中构建治疗响应轨迹，仅使用早期影像数据就能实现高精度的pCR预测。


<details>
  <summary>Details</summary>
Motivation: 个体化治疗决策需要能够预测患者对治疗反应的模型。由于疾病进展和治疗反应在患者间差异很大，这具有挑战性。研究旨在从影像数据中学习治疗早期动态表征来预测乳腺癌患者对新辅助化疗的病理完全缓解。

Method: 使用多任务学习模型从纵向MRI数据中学习潜在空间中的治疗响应轨迹。该模型同时处理外观表征、促进时间连续性，并考虑非响应者群体的高度异质性。在ISPY-2数据集上，通过在潜在轨迹空间中使用线性分类器进行预测。

Result: 在ISPY-2数据集上的实验显示：仅使用治疗前数据(T0)时平衡准确率为0.761，使用早期响应数据(T0+T1)时为0.811，使用四个影像时间点(T0->T3)时达到0.861的平衡准确率。

Conclusion: 该方法能够从治疗早期的MRI影像动态变化中有效学习表征，为预测乳腺癌患者对新辅助化疗的病理完全缓解提供了有前景的工具，代码将在论文接受后公开。
...
Conclusion: 该方法成功可视化了姿态估计网络的决策过程，有助于提高数据驱动航天器姿态估计方法的可解释性和实际应用
...
Method: 评估三个CLIP微调实例（FR、MAD、PAD），在14个通用视觉数据集上进行零样本和线性探测测试，同时使用FR、MAD、PAD基准测试。分析任务复杂度和分类头设计对灾难性遗忘的影响。

Result: 微调模型确实存在过度专业化，特别是复杂的人脸识别任务。FRoundation模型在IJB-C基准上提升58.52%，但在ImageNetV2上性能从69.84%降至51.63%。更大的CLIP架构能更好地保持原始泛化能力。

Conclusion: 基础模型微调会带来过度专业化问题，任务复杂度和分类头设计影响遗忘程度，模型容量增加有助于缓解过度专业化，需要在专业性能和泛化能力之间权衡。
...
Conclusion: GenKOL提供了一个高效、灵活的解决方案，使营销专业人士能够利用生成式AI技术快速创建高质量的虚拟KOL内容，适应不同的使用场景和计算环境。
...
TL;DR: DF-LLaVA是一个简单而有效的框架，通过从MLLMs中提取潜在知识并通过提示注入训练，在保持MLLMs可解释性的同时，实现了超越专家模型的合成图像检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有检测模型主要关注简单的真实性分类，只能提供伪造概率或二元判断，对图像真实性的解释性有限。MLLM-based检测方法虽然能提供更可解释的结果，但在纯真实性分类准确性方面仍落后于专家模型。

Method: 提出DF-LLaVA框架，首先从MLLMs中提取潜在知识，然后通过提示注入到训练中，释放MLLMs的内在判别潜力。
...
Result: 在广泛实验中显著改善了3D少样本类增量学习，实现了优越的几何一致性和对纹理偏差的鲁棒性
...
TL;DR: 提出了基于双曲几何的Brain-HGCN框架，用于fMRI脑网络分析，通过双曲空间建模脑网络层次结构，在精神病分类任务上显著优于欧几里得基线方法


<details>
  <summary>Details</summary>
Motivation: 标准欧几里得GNN难以无失真地表示脑网络的层次拓扑结构，限制了临床性能。脑网络具有层次拓扑结构，对认知处理至关重要
...
TL;DR: RoboEye是一个两阶段目标识别框架，通过动态结合2D语义特征和领域适应的3D推理来解决电商仓库自动化包装中的物体识别难题，在仅使用RGB图像的情况下显著提升识别准确率。


<details>
  <summary>Details</summary>
Motivation: 大规模电商中产品类别快速增长导致物体识别困难，包括类内变异性增加、长尾稀有物品、视觉相似物品、多样化包装、遮挡和大视角变化等问题，这些因素导致仅依赖2D外观特征的方法性能急剧下降。

Method: 提出两阶段识别框架：第一阶段训练大型视觉模型提取2D特征生成候选排名；第二阶段使用轻量级3D特征感知模块评估3D特征质量并预测是否需要3D重排序，避免不必要计算。当需要时，使用机器人3D检索变换器，包括3D特征提取器和基于关键点的匹配器。

Result: 实验显示RoboEye比之前最先进的方法(RoboLLM)在Recall@1上提升了7.1%，且仅使用RGB图像，避免了显式3D输入的依赖，降低了部署成本。

Conclusion: RoboEye通过动态结合2D和3D推理有效解决了电商仓库自动化包装中的物体识别挑战，在保持部署简便性的同时显著提升了识别性能。
...
Method: 采用双流掩码方法：3D空间网格掩码通过坐标排序创建结构化模式来捕捉跨不同方向持续的几何关系；语义掩码使用注意力驱动聚类来发现语义有意义的部件并在掩码过程中保持其连贯性。通过课程学习和动态加权进行协调，从几何理解逐步过渡到语义发现。
...
TL;DR: EchoVLM是一个专门为超声医学影像设计的视觉语言模型，采用混合专家架构，在七个解剖区域数据上训练，能够执行超声报告生成、诊断和视觉问答等多任务，在报告生成任务上相比Qwen2-VL有显著提升。
...
Method: 提出EchoVLM模型，采用混合专家(MoE)架构，在涵盖七个解剖区域的数据集上进行训练，支持超声报告生成、诊断和视觉问答(VQA)等多任务处理。

Result: 实验结果显示，EchoVLM在超声报告生成任务上相比Qwen2-VL，BLEU-1分数提升10.15分，ROUGE-1分数提升4.77分，表现显著优于现有模型。
...
Result: 模型能够从任意视角生成保持空间一致性的多模态输出，在实验中一致优于先前方法。

Conclusion: 通过开源数据和模型，推动了室内场景理解和生成领域的发展，为解决3D室内场景生成的关键瓶颈提供了有效方案。
...
TL;DR: PRISM是一种用于零售场景产品检索的混合方法，结合了视觉语言模型和像素级匹配的优势，通过三阶段流程实现高效精准检索


<details>
  <summary>Details</summary>
Motivation: 传统产品检索面临挑战：不同品牌的同类产品外观高度相似，查询图像视角与目录图像差异大。基础模型难以区分细微局部差异，而像素级匹配方法计算成本高、匹配时间长

Method: 三阶段混合方法：1) 使用SigLIP视觉语言模型检索前35个语义相似产品，大幅缩小搜索空间；2) 应用YOLO-E分割模型消除背景干扰；3) 对筛选候选使用LightGlue进行细粒度像素级匹配

Result: 在ABV数据集上的实验表明，PRISM在top-1准确率上比最先进的图像检索方法提升4.21%，同时保持实时处理能力，适合实际零售部署

Conclusion: PRISM通过结合语义相似性检索和细粒度像素匹配，能够更准确地区分具有高类间相似性的产品，关注全局模型常忽略的细微视觉线索，在效率和准确性之间取得良好平衡
...
<details>
  <summary>Details</summary>
Motivation: 在完全自主无人机中，准确检测障碍物对安全导航至关重要。电线由于其细长轮廓而成为独特且复杂的问题，需要专门解决方案
...
TL;DR: 本文提出了一种改进的水下合成数据生成管道，包含常被忽略的前向散射项和非均匀介质考虑，并收集了BUCKET数据集用于验证。在混浊度增加的情况下，该方法相比参考模型有显著改进，调查参与者选择率达到82.5%。


<details>
  <summary>Details</summary>
Motivation: 现有的水下图像生成模型主要关注变色问题，但忽略了混浊环境中距离相关的可见度损失，特别是前向散射项和非均匀介质的影响。

Method: 提出改进的合成数据生成管道，包含前向散射项和非均匀介质建模，并收集了受控混浊条件下的BUCKET数据集用于验证。

Result: 在混浊度增加的情况下，该方法相比参考模型有定性改进，调查参与者选择率达到82.5%。

Conclusion: 包含前向散射项和非均匀介质考虑的水下图像生成模型能更好地模拟高混浊环境，改进的合成数据生成管道有效提升了水下图像质量。
...
<details>
  <summary>Details</summary>
Motivation: 临床实践中多模态MRI经常存在缺失模态的问题，限制了依赖完整输入的深度学习方法的鲁棒性和泛化能力，特别是在非主导模态组合下。

Method: 提出AdaMM框架，包含三个模块：图引导自适应精炼模块建模通用和模态特定特征间的语义关联；双向瓶颈蒸馏模块通过全局风格匹配和对抗特征对齐传递知识；病变存在引导可靠性模块通过辅助分类任务预测病变类型先验概率。

Result: 在BraTS 2018和2024数据集上的大量实验表明，AdaMM在单模态和弱模态配置下始终优于现有方法，展现出优异的分割准确性和鲁棒性。

Conclusion: 该方法有效解决了多模态脑肿瘤分割中的缺失模态问题，并通过系统评估六类缺失模态策略，确认了知识蒸馏的优越性，为方法选择和未来研究提供了实用指导。
...
Method: 将超参数搜索建模为扩散去噪过程中的序列决策任务，建立马尔可夫决策过程，通过近端策略优化动态调整各去噪步骤的超参数，并将编辑目标整合到奖励函数中

Result: 实验证明相比传统暴力搜索方法，该方法显著减少了搜索时间和计算开销
...
TL;DR: 使用纯合成数据和域随机化策略训练YOLOv11模型检测特定物体（汤罐），通过增加数据集多样性和精心调优的数据增强来弥合合成到真实的域差距，最佳模型在Kaggle竞赛隐藏测试集上达到0.910 mAP@50。
...
Method: 采用YOLOv11模型，通过广泛实验数据增强、数据集组合和模型缩放，使用合成数据和域随机化策略进行训练，并通过定量和定性评估指导开发。

Result: 最佳配置的YOLOv11l模型在扩展和多样化的数据集上训练，在竞赛隐藏测试集上达到0.910 mAP@50，合成验证指标高但无法准确预测真实世界性能。

Conclusion: 纯合成训练方法具有潜力，增加合成数据集多样性（包括不同视角和复杂背景）与精心调优的数据增强对弥合域差距至关重要，但仍面临完全捕捉真实世界变异性的挑战。
...
TL;DR: 本研究评估了三种公开的深度学习肺部分割模型在肺移植患者中的表现，发现Unet-R231在整体性能上最优，但所有模型在重度病例中性能显著下降。
...
Method: 回顾性研究32例患者的3645张CT切片，使用Unet-R231、TotalSegmentator和MedSAM三种模型进行肺部分割，采用定量指标（体积相似性、Dice系数、Hausdorff距离）和定性临床可接受度评分进行评估。

Result: Unet-R231在所有评估维度均表现最佳，但所有模型从轻度到中重度病例性能显著下降（p<0.05），肺侧和病理类型间无显著差异。

Conclusion: Unet-R231是最准确的自动化肺部分割模型，但在重度病理情况下需要专门的模型微调，TotalSegmentator可作为次优选择。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究证明多模态线索对鲁棒语义分割有益，但缺乏灵活的预训练-微调多模态视觉处理流程

Method: 1) 基于ImageNet构建包含5种视觉模态的大规模多模态预训练数据集ImageNeXt；2) 提出高效预训练方法使模型具备编码不同模态信息的能力
...
<details>
  <summary>Details</summary>
Motivation: COLMAP在动态场景中需要真实运动掩码且运行时间长，现有方法依赖多种难以获取的先验信息（如真实焦距、运动掩码、3D点云等），限制了在普通RGB视频中的应用

Method: 包含三个关键组件：(1) 块状跟踪滤波器建立稀疏铰链关系；(2) 异常值感知联合优化，自适应降权移动异常值；(3) 两阶段优化策略，通过Softplus限制和凸最小值的权衡提升稳定性和速度
...
TL;DR: MEDFACT-R1是一个两阶段医疗视觉语言模型框架，通过外部知识基础和强化学习提升医学事实推理能力，在三个医疗QA基准测试中实现22.5%的绝对准确率提升
...
Conclusion: 伪标签SFT冷启动的必要性和每个GRPO奖励的贡献得到验证，知识基础与RL驱动推理的协同作用对于可信医疗AI至关重要
...
<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型主要依赖大数据集的统计规律，但很少融入感知心理学的结构化洞察。本研究探索将人类感知中经典几何视觉错觉作为归纳偏置的潜力

Method: 创建参数化几何错觉合成数据集，评估三种多源学习策略（结合错觉识别任务和ImageNet分类目标），在CNN和transformer架构上进行实验

Result: 几何错觉作为辅助监督能系统改善泛化性能；感知驱动的归纳偏置能增强模型对结构的敏感性，即使这些偏置来自与自然图像识别传统无关的合成刺激

Conclusion: 展示了感知科学与机器学习的新颖整合，为将感知先验嵌入视觉模型设计提供了新方向
...
Method: 采用多样化查询生成策略模拟真实用户查询的语言变化，并基于遗传算法进行联合优化，平衡攻击成功率、清洁任务效用和隐蔽性来演化对抗性提示。

Result: 实验结果显示AIP攻击成功率高达95.23%，同时保持了良性功能，揭示了RAG系统中一个关键且先前被忽视的漏洞。

Conclusion: 研究结果强调了需要重新评估共享指令提示的安全性，指令提示中的信任组件可能被武器化来破坏系统完整性。
...
TL;DR: 提出M&N框架，通过从2D预训练模型向3D分割模型渐进式知识蒸馏，解决半监督3D医学图像分割问题，在多个数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 解决3D医学图像标注数据稀缺问题，利用丰富的2D自然图像预训练模型知识来提升半监督3D分割性能

Method: 模型无关框架，通过迭代协同训练和伪掩码生成，结合学习率引导采样自适应调整标注和未标注数据比例

Result: 在多个公开数据集上超越13种现有半监督分割方法，达到最先进性能，且具有良好的模型适应性

Conclusion: M&N框架有效实现了从2D到3D的知识迁移，为半监督医学图像分割提供了有效的解决方案，具有很好的通用性和适应性
...
TL;DR: 提出了一种名为RA-GAN的无种族偏见人脸老化模型，通过RACEpSp和特征混合器模块生成无偏图像，用于亲属验证中同年龄父母-子女图像的验证，显著提升了准确率。


<details>
  <summary>Details</summary>
Motivation: 解决亲属验证中的年龄差距问题，传统人脸老化模型存在种族偏见，影响图像相似度，且同年龄照片往往不可得。

Method: 开发RA-GAN模型，包含RACEpSp和特征混合器两个新模块，生成无种族偏见的合成图像，用于同年龄亲属验证。

Result: RA-GAN在所有年龄组平均比SAM-GAN提升13.14%，在60+年龄组比CUSP-GAN提升9.1%的种族准确性。身份保持效果更好，在KinFaceW-I和KinFaceW-II数据集上亲属验证准确率均有提升。

Conclusion: RA-GAN能有效生成无种族偏见的人脸老化图像，显著提升亲属验证准确率，特别是在处理年龄差距大的亲属关系时效果显著。
...
TL;DR: 本文提出了一种基于多模态大语言模型（MLLMs）的零样本时空视频定位框架，通过分解时空高亮和时间增强组装策略来提升MLLMs在时空视频定位任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 探索利用MLLMs实现零样本时空视频定位（STVG），发现MLLMs存在两个关键问题：1）倾向于动态分配特殊标记进行文本查询定位；2）由于无法充分整合文本查询中的线索（如属性、动作）进行推理，导致定位效果不佳。

Method: 提出DSTH策略将原始查询分解为属性和动作子查询，使用logit引导重注意模块学习空间和时间提示；引入TAS策略通过原始视频帧和时间增强帧的输入组装预测结果，提高时间一致性。

Result: 在三个常见STVG基准测试中，该方法优于最先进的方法。

Conclusion: 该方法有效释放了MLLMs的推理能力，为时空视频定位提供了有效的零样本解决方案。
...
TL;DR: MSDD是一个高质量玉米幼苗航空图像数据集，用于精准农业中的植株计数，包含单株、双株和三株三类，在V4-V6生长阶段和天底视角下检测效果最佳。


<details>
  <summary>Details</summary>
Motivation: 精准农业需要准确的玉米幼苗检测，但现有数据集稀缺。传统人工计数方法劳动密集且易出错，计算机视觉技术可以提供高效准确的检测方案。

Method: 构建MSDD数据集，包含三类玉米植株（单株、双株、三株），涵盖不同生长阶段、种植设置、土壤类型、光照条件和拍摄角度。使用YOLO系列模型进行基准测试。

Result: 单株检测精度最高（精确度0.984，召回率0.873），YOLOv9在单株检测中准确率最高，YOLO11推理速度最快（35ms/图像）。双株和三株检测因稀有性和外观不规则而困难。
...
<details>
  <summary>Details</summary>
Motivation: 自回归模型在视觉领域面临局部条件依赖、步间语义不一致和空间不变性不足等问题，限制了高质量视觉表示的学习能力。

Method: 提出ST-AR训练框架，在训练过程中引入自监督目标，无需依赖预训练表示模型，有效解决上述三个关键问题。

Result: ST-AR显著提升了自回归模型的图像理解能力，LlamaGen-L和LlamaGen-XL的FID分别提升了约42%和49%，同时保持相同的采样策略。

Conclusion: 自监督目标的引入能够有效解决自回归模型在视觉领域的核心挑战，为高质量视觉表示学习提供了新的训练范式。
...
TL;DR: SyncSeal是一种专门用于图像同步的水印方法，通过在现有水印方法基础上添加几何变换估计功能，增强对裁剪、旋转等几何变换的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有水印方法在面对几何变换时容易失效，需要一种能够准确估计和反转几何变换的同步机制来提升水印的鲁棒性。

Method: 使用嵌入网络对图像进行不可感知的修改，提取网络预测图像所受的几何变换参数，结合判别器保持感知质量，端到端训练最小化预测误差。

Result: 实验验证了该方法在多种几何和值度变换下的有效性，能够准确同步图像，并成功提升现有水印方法对几何变换的抵抗能力。

Conclusion: SyncSeal为图像水印提供了有效的同步解决方案，显著增强了水印系统对几何攻击的鲁棒性。
...
TL;DR: RynnVLA-001是一个基于大规模视频生成预训练的视觉-语言-动作模型，采用两阶段预训练方法：第一阶段通过自我中心视频生成预训练预测未来帧，第二阶段通过人类中心轨迹感知建模联合预测关键点轨迹，并使用ActionVAE压缩动作序列，在下游机器人任务中表现优于现有基准。


<details>
  <summary>Details</summary>
Motivation: 为了解决视觉-语言-动作模型在复杂动作空间中的表示和预测问题，需要开发更有效的预训练策略来桥接视觉帧预测和动作预测之间的鸿沟。

Method: 提出两阶段预训练方法：1）自我中心视频生成预训练阶段，在1200万自我中心操作视频上训练图像到视频模型；2）人类中心轨迹感知建模阶段，联合预测未来关键点轨迹；同时使用ActionVAE变分自编码器压缩动作序列。

Result: 在相同下游机器人数据集上微调后，RynnVLA-001实现了优于最先进基准模型的性能表现。

Conclusion: 所提出的预训练策略为视觉-语言-动作模型提供了更有效的初始化方法，证明了视频生成预训练在VLA模型中的有效性。
...
TL;DR: 本文提出了OST方法，用于预测视野外物体的无噪声视觉轨迹，通过视觉-定位去噪模块处理噪声传感器数据，在Vi-Fi和JRDB数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测方法依赖完整无噪声观测数据，忽视了视野外物体和传感器噪声的挑战，这些限制在实际场景中带来安全风险并阻碍可靠预测。

Method: 增强的视觉-定位去噪模块利用相机标定建立视觉-定位映射，以无监督方式有效去噪噪声传感器数据，扩展应用到行人和车辆的视野外轨迹预测。

Result: 在Vi-Fi和JRDB数据集上的广泛评估显示，该方法在轨迹去噪和预测方面都达到了最先进性能，显著超越了之前的基线方法。

Conclusion: 这是首个整合视觉-定位投影来去噪视野外智能体噪声传感器轨迹的工作，为未来进展铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 传统多视角立体视觉方法计算效率低，而扩散模型在生成任务中表现出色，希望通过引入扩散模型来提升MVS的效率和性能
...
Conclusion: 扩散模型成功应用于MVS任务，提出的框架在效率和性能方面都表现出色，为3D几何重建提供了新的解决方案
...
TL;DR: 提出了一种基于视觉基础模型的跨模态蒸馏方法，用于从事件相机数据中进行单目深度估计，无需密集深度标注，在合成和真实数据集上达到竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 事件相机在高速运动和强光照变化环境下具有优势，但缺乏大规模带密集深度标注的数据集，阻碍了基于学习的单目深度估计方法发展。

Method: 使用跨模态蒸馏范式，利用视觉基础模型生成密集代理标签；提出两种VFM模型：基于Depth Anything v2的普通版本和新型循环架构。

Result: 在合成和真实数据集上评估表明：跨模态蒸馏方法无需昂贵深度标注即可达到与全监督方法竞争的性能；基于VFM的模型实现了最先进的性能。

Conclusion: 该方法有效解决了事件相机深度估计中标注数据稀缺的问题，通过跨模态蒸馏和VFM模型实现了优异的性能，为事件相机的深度感知应用提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决VLMs在开放词汇语义分割中的无源域适应问题，需要在不访问源数据的情况下提升模型在新域上的性能，同时保持计算效率。
...
TL;DR: CalibPrompt是一个在提示调优过程中校准医学视觉-语言模型置信度的框架，通过设计校准目标和角度分离损失来改善模型校准性能，在保持准确性的同时显著提升置信度可靠性。
...
Method: 提出CalibPrompt框架，在稀缺标注数据下优化可学习提示集，使用校准目标包括：1）对齐平滑准确率与预测置信度的正则化器；2）最大化文本特征接近度的角度分离损失，提升多模态医学视觉-语言模型的置信度估计可靠性。

Result: 在4个公开医学视觉-语言模型和5个多样化医疗影像数据集上的广泛实验表明，CalibPrompt能够一致地改善校准性能，同时不会显著影响原始准确率。

Conclusion: CalibPrompt是首个在提示调优过程中校准医学视觉-语言模型的框架，有效解决了置信度校准问题，为医疗AI系统的可靠部署提供了重要技术支撑。
